// app/design/components/Menu/native/MenuGroup.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    romeo = 0;
    golf = oscar[romeo];
    tango = argCorge;
    entity = undefined;
    golf = tango.bind(entity)(golf);
    var _closure1_slot0 = golf;
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    yankee = tango.StyleSheet;
    tango = tango.View;
    var _closure1_slot1 = tango;
    tango = 2;
    options = oscar[tango];
    options = report.bind(entity)(options);
    options = options.jsx;
    var _closure1_slot2 = options;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsxs;
    var _closure1_slot3 = tango;
    tango = 3;
    tango = oscar[tango];
    verify = report.bind(entity)(tango);
    options = verify.createStyles;
    tango = {};
    offset = {};
    offset['marginLeft'] = romeo;
    romeo = yankee.hairlineWidth;
    offset['height'] = romeo;
    romeo = 4;
    foxtrot = oscar[romeo];
    romeo = argBaz;
    romeo = romeo.bind(entity)(foxtrot);
    romeo = romeo.colors;
    romeo = romeo.BACKGROUND_MODIFIER_ACCENT;
    offset['backgroundColor'] = romeo;
    romeo = yankee.hairlineWidth;
    yankee = -1;
    yankee = yankee * romeo;
    offset['marginTop'] = yankee;
    tango['divider'] = offset;
    tango = options.bind(verify)(tango);
    var _closure1_slot4 = tango;
    tango = golf.forwardRef;
    mike = function(argFoo, argBar) {
        _fun48595: for(var _fun48595_ip = 0; ; ) switch(_fun48595_ip) {
 0:
            entity = argFoo;
            options = argBar;
            report = entity.style;
            verify = entity.children;
            var _closure2_slot0 = options;
            entity = _closure1_slot4;
            tango = undefined;
            romeo = entity.bind(tango)();
            zulu = _closure1_slot3;
            mike = _closure1_slot1;
            entity = {};
            entity['style'] = report;
            report = null;
            report = report !== options;
            options = !report;
            if(report) { _fun48595_ip = 87; continue _fun48595 }
 61:
            yankee = _closure1_slot2;
            offset = _closure1_slot1;
            report = {};
            romeo = romeo.divider;
            report['style'] = romeo;
            options = yankee.bind(tango)(offset, report);
 87:
            report = new Array(2);
            report[0] = options;
            golf = _closure1_slot0;
            options = golf.Children;
            golf = options.map;
            oscar = function(argFoo, argBar) {
                _fun48596: for(var _fun48596_ip = 0; ; ) switch(_fun48596_ip) {
 0:
                    report = argFoo;
                    zulu = 0;
                    mike = argBar;
                    entity = report;
                    if(!(zulu === mike)) { _fun48596_ip = 68; continue _fun48596 }
 15:
                    tango = _closure1_slot0;
                    zulu = tango.isValidElement;
                    zulu = zulu.bind(tango)(report);
                    entity = report;
                    if(!zulu) { _fun48596_ip = 68; continue _fun48596 }
 39:
                    tango = _closure1_slot0;
                    zulu = tango.cloneElement;
                    mike = {};
                    oscar = _closure2_slot0;
                    mike['ref'] = oscar;
                    entity = zulu.bind(tango)(report, mike);
 68:
                    return entity;
                }
            };
            oscar = golf.bind(options)(verify, oscar);
            report[1] = oscar;
            entity['children'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    mike = tango.bind(golf)(mike);
    tango = 5;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'design/components/Menu/native/MenuGroup.tsx';
    tango = report.bind(oscar)(tango);
    zulu['MenuGroup'] = mike;
    return entity;
})();