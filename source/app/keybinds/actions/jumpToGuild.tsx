// app/keybinds/actions/jumpToGuild.tsx
export default (function(argFoo, argBar, argBaz, _, __, argFred, argPlugh) {
    _fun105744: for(var _fun105744_ip = 0; ; ) switch(_fun105744_ip) {
 0:
        oscar = argBar;
        options = argBaz;
        zulu = argFred;
        golf = argPlugh;
        var _closure1_slot0 = oscar;
        var _closure1_slot1 = golf;
        romeo = global;
        offset = romeo.Object;
        verify = offset.defineProperty;
        mike = {};
        report = true;
        mike['value'] = report;
        entity = '__esModule';
        entity = verify.bind(offset)(zulu, entity, mike);
        entity = 0;
        mike = golf[entity];
        entity = undefined;
        mike = options.bind(entity)(mike);
        var _closure1_slot2 = mike;
        yankee = 1;
        mike = golf[yankee];
        mike = options.bind(entity)(mike);
        var _closure1_slot3 = mike;
        mike = 2;
        mike = golf[mike];
        mike = options.bind(entity)(mike);
        var _closure1_slot4 = mike;
        mike = 3;
        mike = golf[mike];
        mike = options.bind(entity)(mike);
        var _closure1_slot5 = mike;
        mike = 4;
        mike = golf[mike];
        mike = options.bind(entity)(mike);
        var _closure1_slot6 = mike;
        mike = 5;
        mike = golf[mike];
        mike = oscar.bind(entity)(mike);
        options = mike.ME;
        var _closure1_slot7 = options;
        options = mike.FAVORITES;
        var _closure1_slot8 = options;
        mike = mike.Routes;
        var _closure1_slot9 = mike;
        mike = {};
        options = new Array(0);
        offset = 'mod+';
        verify = 10;
 192:
        backup = options.push;
        foxtrot = romeo.HermesInternal;
        foxtrot = foxtrot.concat;
        foxtrot = foxtrot.bind(offset)(yankee);
        foxtrot = backup.bind(options)(foxtrot);
        yankee = yankee + 1;
        if(yankee < verify) { _fun105744_ip = 192; continue _fun105744 }
 225:
        mike['binds'] = options;
        mike['comboKeysBindGlobal'] = report;
        tango = function(argFoo, argBar) { // Original name: action
            _fun105745: for(var _fun105745_ip = 0; ; ) switch(_fun105745_ip) {
 0:
                zulu = argBar;
                tango = global;
                report = tango.parseInt;
                mike = zulu.split;
                entity = '+';
                entity = mike.bind(zulu)(entity);
                mike = 1;
                zulu = entity[mike];
                entity = undefined;
                oscar = 10;
                report = report.bind(entity)(zulu, oscar);
                zulu = 0;
                if(!(zulu === report)) { _fun105745_ip = 52; continue _fun105745 }
 49:
                report = oscar;
 52:
                tango = tango.isNaN;
                tango = tango.bind(entity)(report);
                if(tango) { _fun105745_ip = 323; continue _fun105745 }
 69:
                if(!(zulu !== report)) { _fun105745_ip = 323; continue _fun105745 }
 76:
                if(!(mike !== report)) { _fun105745_ip = 198; continue _fun105745 }
 80:
                tango = _closure1_slot0;
                oscar = _closure1_slot1;
                mike = 6;
                mike = oscar[mike];
                options = tango.bind(entity)(mike);
                golf = options.isFavoritesGuildVisible;
                oscar = _closure1_slot2;
                tango = _closure1_slot3;
                mike = _closure1_slot4;
                tango = golf.bind(options)(oscar, tango, mike);
                mike = 2;
                if(tango) { _fun105745_ip = 158; continue _fun105745 }
 134:
                oscar = _closure1_slot6;
                tango = oscar.getFlattenedGuildIds;
                oscar = tango.bind(oscar)();
                tango = report - mike;
                oscar = oscar[tango];
                _fun105745_ip = 196; continue _fun105745;
 158:
                if(!(mike !== report)) { _fun105745_ip = 189; continue _fun105745 }
 162:
                tango = _closure1_slot6;
                mike = tango.getFlattenedGuildIds;
                tango = mike.bind(tango)();
                mike = 3;
                mike = report - mike;
                mike = tango[mike];
                _fun105745_ip = 193; continue _fun105745;
 189:
                mike = _closure1_slot8;
 193:
                oscar = mike;
 196:
                _fun105745_ip = 205; continue _fun105745;
 198:
                oscar = _closure1_slot7;
 205:
                tango = null;
                if(!(tango != oscar)) { _fun105745_ip = 323; continue _fun105745 }
 211:
                zulu = _closure1_slot7;
                if(!(oscar === zulu)) { _fun105745_ip = 240; continue _fun105745 }
 222:
                report = _closure1_slot5;
                zulu = report.getGuildId;
                zulu = zulu.bind(report)();
                if(!(tango != zulu)) { _fun105745_ip = 273; continue _fun105745 }
 240:
                tango = _closure1_slot0;
                report = _closure1_slot1;
                zulu = 8;
                zulu = report[zulu];
                tango = tango.bind(entity)(zulu);
                zulu = tango.transitionToGuild;
                zulu = zulu.bind(tango)(oscar);
                _fun105745_ip = 319; continue _fun105745;
 273:
                tango = _closure1_slot0;
                report = _closure1_slot1;
                zulu = 7;
                zulu = report[zulu];
                tango = tango.bind(entity)(zulu);
                zulu = tango.transitionTo;
                report = _closure1_slot9;
                mike = report.CHANNEL;
                mike = mike.bind(report)(oscar);
                mike = zulu.bind(tango)(mike);
 319:
                mike = false;
                return mike;
 323:
                return entity;
            }
        };
        mike['action'] = tango;
        tango = {};
        tango['JUMP_TO_GUILD'] = mike;
        report = 9;
        report = golf[report];
        golf = oscar.bind(entity)(report);
        oscar = golf.fileFinishedImporting;
        report = 'keybinds/actions/jumpToGuild.tsx';
        report = oscar.bind(golf)(report);
        zulu['default'] = tango;
        zulu['JUMP_TO_GUILD'] = mike;
        return entity;
    }
})();