// app/modules/instant_invite/native/components/InstantInviteShareApps.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    romeo = argBaz;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = romeo;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    tango = oscar[entity];
    entity = undefined;
    tango = romeo.bind(entity)(tango);
    var _closure1_slot3 = tango;
    tango = 1;
    golf = oscar[tango];
    tango = argCorge;
    golf = tango.bind(entity)(golf);
    var _closure1_slot4 = golf;
    tango = 2;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    options = tango.View;
    var _closure1_slot5 = options;
    tango = tango.ScrollView;
    var _closure1_slot6 = tango;
    tango = 3;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    options = tango.SHARE_ITEMS;
    var _closure1_slot7 = options;
    tango = tango.SHARE_ITEMS_DEFAULT;
    var _closure1_slot8 = tango;
    tango = 4;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsx;
    var _closure1_slot9 = tango;
    tango = 5;
    tango = oscar[tango];
    verify = report.bind(entity)(tango);
    options = verify.createStyles;
    tango = {};
    offset = {};
    yankee = 6;
    foxtrot = oscar[yankee];
    foxtrot = romeo.bind(entity)(foxtrot);
    foxtrot = foxtrot.spacing;
    foxtrot = foxtrot.PX_16;
    offset['padding'] = foxtrot;
    yankee = oscar[yankee];
    yankee = romeo.bind(entity)(yankee);
    yankee = yankee.spacing;
    yankee = yankee.PX_12;
    offset['gap'] = yankee;
    yankee = 'center';
    offset['alignItems'] = yankee;
    tango['contentContainer'] = offset;
    tango = options.bind(verify)(tango);
    var _closure1_slot10 = tango;
    tango = golf.memo;
    mike = function(argFoo) { // Original name: InstantInviteShareApps
        entity = argFoo;
        mike = entity.onItemPressed;
        var _closure2_slot0 = mike;
        options = entity.contentContainerStyle;
        mike = _closure1_slot10;
        tango = undefined;
        oscar = mike.bind(tango)();
        zulu = _closure1_slot0;
        golf = _closure1_slot2;
        mike = 7;
        mike = golf[mike];
        zulu = zulu.bind(tango)(mike);
        mike = zulu.useFontScale;
        mike = mike.bind(zulu)();
        var _closure2_slot1 = mike;
        offset = _closure1_slot4;
        zulu = offset.useState;
        mike = _closure1_slot8;
        golf = zulu.bind(offset)(mike);
        zulu = _closure1_slot3;
        mike = 2;
        zulu = zulu.bind(tango)(golf, mike);
        mike = 0;
        golf = zulu[mike];
        mike = 1;
        mike = zulu[mike];
        var _closure2_slot2 = mike;
        verify = offset.useEffect;
        zulu = function() {
            mike = global;
            tango = mike.Promise;
            zulu = tango.all;
            oscar = _closure1_slot7;
            report = oscar.map;
            mike = function(argFoo) {
                entity = argFoo;
                entity = entity.isAvailable;
                return entity;
            };
            mike = report.bind(oscar)(mike);
            zulu = zulu.bind(tango)(mike);
            mike = zulu.then;
            entity = function(argFoo) {
                tango = argFoo;
                zulu = new Array(0);
                var _closure4_slot0 = zulu;
                mike = tango.forEach;
                entity = function(argFoo, argBar) {
                    _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
                        entity = argFoo;
                        if(!entity) { _fun00002_ip = 37; continue _fun00001 }
 6:
                        zulu = _closure4_slot0;
                        mike = zulu.push;
                        tango = _closure1_slot7;
                        entity = argBar;
                        entity = tango[entity];
                        entity = mike.bind(zulu)(entity);
 37:
                        entity = undefined;
                        return entity;
                    }
                };
                entity = mike.bind(tango)(entity);
                mike = _closure2_slot2;
                entity = undefined;
                mike = mike.bind(entity)(zulu);
                return entity;
            };
            entity = mike.bind(zulu)(entity);
            entity = undefined;
            return entity;
        };
        mike = new Array(0);
        mike = verify.bind(offset)(zulu, mike);
        zulu = _closure1_slot9;
        mike = _closure1_slot6;
        entity = {'contentContainerStyle': null, 'showsHorizontalScrollIndicator': false, 'horizontal': true};
        verify = oscar.contentContainer;
        oscar = new Array(2);
        oscar[0] = verify;
        oscar[1] = options;
        entity['contentContainerStyle'] = oscar;
        oscar = golf.map;
        report = function(argFoo) {
            _fun00003: for(var _fun00004_ip = 0; ; ) switch(_fun00004_ip) {
 0:
                entity = argFoo;
                romeo = entity.fullIcon;
                yankee = entity.getLabel;
                backup = entity.icon;
                foxtrot = entity.IconComponent;
                mike = entity.onPress;
                var _closure3_slot0 = mike;
                report = entity.type;
                tango = _closure1_slot9;
                zulu = _closure1_slot5;
                mike = {};
                entity = {};
                options = _closure2_slot1;
                golf = 76;
                golf = golf * options;
                entity['maxWidth'] = golf;
                mike['style'] = entity;
                kilo = null;
                if(!(kilo == romeo)) { _fun00004_ip = 198; continue _fun00003 }
 86:
                verify = _closure1_slot9;
                golf = _closure1_slot0;
                options = _closure1_slot2;
                entity = 9;
                entity = options[entity];
                options = undefined;
                entity = golf.bind(options)(entity);
                golf = entity.IconButton;
                entity = {};
                sizing = 'secondary';
                entity['variant'] = sizing;
                if(!(kilo == foxtrot)) { _fun00004_ip = 159; continue _fun00003 }
 132:
                if(!(kilo == backup)) { _fun00004_ip = 156; continue _fun00003 }
 136:
                sizing = _closure1_slot1;
                output = _closure1_slot2;
                kilo = 10;
                kilo = output[kilo];
                backup = sizing.bind(options)(kilo);
 156:
                foxtrot = backup;
 159:
                entity['icon'] = foxtrot;
                foxtrot = yankee.bind(options)();
                entity['label'] = foxtrot;
                foxtrot = function() { // Original name: onPress
                    zulu = _closure2_slot0;
                    mike = _closure3_slot0;
                    entity = undefined;
                    entity = zulu.bind(entity)(mike);
                    return entity;
                };
                entity['onPress'] = foxtrot;
                foxtrot = 2;
                entity['maxFontSizeMultiplier'] = foxtrot;
                entity = verify.bind(options)(golf, entity);
                _fun00004_ip = 270; continue _fun00003;
 198:
                verify = _closure1_slot9;
                golf = _closure1_slot0;
                options = _closure1_slot2;
                oscar = 8;
                oscar = options[oscar];
                options = undefined;
                oscar = golf.bind(options)(oscar);
                golf = oscar.ImageButton;
                oscar = {};
                oscar['image'] = romeo;
                yankee = yankee.bind(options)();
                oscar['label'] = yankee;
                offset = function() { // Original name: onPress
                    zulu = _closure2_slot0;
                    mike = _closure3_slot0;
                    entity = undefined;
                    entity = zulu.bind(entity)(mike);
                    return entity;
                };
                oscar['onPress'] = offset;
                offset = 2;
                oscar['maxFontSizeMultiplier'] = offset;
                entity = verify.bind(options)(golf, oscar);
 270:
                mike['children'] = entity;
                entity = undefined;
                entity = tango.bind(entity)(zulu, mike, report);
                return entity;
            }
        };
        report = oscar.bind(golf)(report);
        entity['children'] = report;
        entity = zulu.bind(tango)(mike, entity);
        return entity;
    };
    mike = tango.bind(golf)(mike);
    tango = 11;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/instant_invite/native/components/InstantInviteShareApps.tsx';
    tango = report.bind(oscar)(tango);
    zulu['default'] = mike;
    return entity;
})();