// app/modules/auth/native/components/RegisterUsernameInput.tsx
export default (function(argFoo, argBar, argBaz, argCor, _, argFre, argPlu) {
    report = argBar;
    golfie = argBaz;
    zuuluu = argFre;
    oscard = argPlu;
    var _closure1_slot0 = report;
    var _closure1_slot1 = golfie;
    var _closure1_slot2 = oscard;
    entity = global;
    verify = entity.Object;
    option = verify.defineProperty;
    tangon = {};
    entity = true;
    tangon['value'] = entity;
    entity = '__esModule';
    entity = option.bind(verify)(zuuluu, entity, tangon);
    entity = 0;
    tangon = oscard[entity];
    entity = undefined;
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot3 = tangon;
    tangon = 1;
    golfie = oscard[tangon];
    tangon = argCor;
    tangon = tangon.bind(entity)(golfie);
    var _closure1_slot4 = tangon;
    tangon = 2;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    golfie = tangon.jsx;
    var _closure1_slot5 = golfie;
    golfie = tangon.jsxs;
    var _closure1_slot6 = golfie;
    tangon = tangon.Fragment;
    var _closure1_slot7 = tangon;
    tangon = 3;
    tangon = oscard[tangon];
    option = report.bind(entity)(tangon);
    golfie = option.createStyles;
    tangon = {};
    verify = {'marginBottom': 8, 'width': '90%'};
    tangon['status'] = verify;
    verify = {'marginBottom': 8, 'width': '100%'};
    tangon['inputHint'] = verify;
    tangon = golfie.bind(option)(tangon);
    var _closure1_slot8 = tangon;
    tangon = {};
    golfie = 4;
    option = oscard[golfie];
    option = report.bind(entity)(option);
    offset = option.FadeIn;
    option = offset.duration;
    verify = 300;
    option = option.bind(offset)(verify);
    tangon['entering'] = option;
    option = oscard[golfie];
    option = report.bind(entity)(option);
    offset = option.FadeOut;
    option = offset.duration;
    option = option.bind(offset)(verify);
    tangon['exiting'] = option;
    var _closure1_slot9 = tangon;
    tangon = {};
    option = oscard[golfie];
    option = report.bind(entity)(option);
    offset = option.LinearTransition;
    option = offset.easing;
    yankee = oscard[golfie];
    yankee = report.bind(entity)(yankee);
    romeon = yankee.Easing;
    yankee = romeon.inOut;
    golfie = oscard[golfie];
    golfie = report.bind(entity)(golfie);
    golfie = golfie.Easing;
    golfie = golfie.quad;
    golfie = yankee.bind(romeon)(golfie);
    option = option.bind(offset)(golfie);
    golfie = option.duration;
    golfie = golfie.bind(option)(verify);
    tangon['layout'] = golfie;
    var _closure1_slot10 = tangon;
    tangon = function(argFoo) { // Original name: UsernameStatusMessage
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            entity = argFoo;
            offset = entity.usernameStatus;
            michal = entity.isUsernameFocused;
            entity = _closure1_slot8;
            report = undefined;
            romeon = entity.bind(report)();
            entity = null;
            zuuluu = entity == offset;
            tangon = undefined;
            if(zuuluu) { _fun00002_ip = 44; continue _fun00001 }
 39:
            tangon = offset.type;
 44:
            oscard = _closure1_slot0;
            zuuluu = _closure1_slot2;
            golfie = 5;
            zuuluu = zuuluu[golfie];
            zuuluu = oscard.bind(report)(zuuluu);
            zuuluu = zuuluu.NameValidationState;
            zuuluu = zuuluu.ERROR;
            if(!(tangon !== zuuluu)) { _fun00002_ip = 474; continue _fun00001 }
 83:
            if(!michal) { _fun00002_ip = 136; continue _fun00001 }
 86:
            zuuluu = entity == offset;
            tangon = undefined;
            if(zuuluu) { _fun00002_ip = 100; continue _fun00001 }
 95:
            tangon = offset.type;
 100:
            oscard = _closure1_slot0;
            zuuluu = _closure1_slot2;
            zuuluu = zuuluu[golfie];
            zuuluu = oscard.bind(report)(zuuluu);
            zuuluu = zuuluu.NameValidationState;
            zuuluu = zuuluu.AVAILABLE;
            if(!(tangon !== zuuluu)) { _fun00002_ip = 320; continue _fun00001 }
 136:
            entity = null;
            if(!michal) { _fun00002_ip = 315; continue _fun00001 }
 144:
            tangon = _closure1_slot5;
            verify = _closure1_slot0;
            foxtra = _closure1_slot2;
            michal = 9;
            michal = foxtra[michal];
            michal = verify.bind(report)(michal);
            zuuluu = michal.Text;
            michal = {};
            sizing = _closure1_slot9;
            output = michal;
            oscard = copyDataProperties(output, sizing);
            sizing = _closure1_slot10;
            output = michal;
            oscard = copyDataProperties(output, sizing);
            golfie = romeon.inputHint;
            oscard = 'style';
            michal[oscard] = golfie;
            golfie = 'text-xs/medium';
            oscard = 'variant';
            michal[oscard] = golfie;
            golfie = 'text-muted';
            oscard = 'color';
            michal[oscard] = golfie;
            golfie = true;
            oscard = 'animated';
            michal[oscard] = golfie;
            oscard = 10;
            golfie = foxtra[oscard];
            golfie = verify.bind(report)(golfie);
            option = golfie.intl;
            golfie = option.string;
            oscard = foxtra[oscard];
            oscard = verify.bind(report)(oscard);
            oscard = oscard.t;
            oscard = oscard.y7LSyc;
            golfie = golfie.bind(option)(oscard);
            oscard = 'children';
            michal[oscard] = golfie;
            entity = tangon.bind(report)(zuuluu, michal);
 315:
            _fun00002_ip = 469; continue _fun00001;
 320:
            tangon = _closure1_slot5;
            golfie = _closure1_slot0;
            option = _closure1_slot2;
            oscard = 9;
            michal = option[oscard];
            michal = golfie.bind(report)(michal);
            zuuluu = michal.Text;
            michal = {};
            sizing = _closure1_slot9;
            output = michal;
            verify = copyDataProperties(output, sizing);
            sizing = _closure1_slot10;
            output = michal;
            verify = copyDataProperties(output, sizing);
            foxtra = romeon.status;
            verify = 'style';
            michal[verify] = foxtra;
            foxtra = 'text-xs/medium';
            verify = 'variant';
            michal[verify] = foxtra;
            foxtra = true;
            verify = 'animated';
            michal[verify] = foxtra;
            oscard = option[oscard];
            oscard = golfie.bind(report)(oscard);
            golfie = oscard.Text;
            oscard = {'variant': 'text-xs/medium', 'color': 'text-positive'};
            option = offset.message;
            oscard['children'] = option;
            golfie = tangon.bind(report)(golfie, oscard);
            oscard = 'children';
            michal[oscard] = golfie;
            entity = tangon.bind(report)(zuuluu, michal);
 469:
            _fun00002_ip = 702; continue _fun00001;
 474:
            tangon = _closure1_slot6;
            option = _closure1_slot0;
            foxtra = _closure1_slot2;
            michal = 6;
            michal = foxtra[michal];
            michal = option.bind(report)(michal);
            zuuluu = michal.Stack;
            michal = {'direction': 'horizontal', 'spacing': 4, 'align': 'flex-start'};
            verify = _closure1_slot5;
            oscard = 7;
            oscard = foxtra[oscard];
            oscard = option.bind(report)(oscard);
            golfie = oscard.CircleWarningIcon;
            oscard = {};
            backup = 'xs';
            oscard['size'] = backup;
            kiloes = _closure1_slot1;
            backup = 8;
            backup = foxtra[backup];
            backup = kiloes.bind(report)(backup);
            backup = backup.colors;
            backup = backup.TEXT_DANGER;
            oscard['color'] = backup;
            golfie = verify.bind(report)(golfie, oscard);
            oscard = new Array(2);
            oscard[0] = golfie;
            golfie = 9;
            golfie = foxtra[golfie];
            golfie = option.bind(report)(golfie);
            option = golfie.Text;
            golfie = {'variant': 'text-xs/medium', 'color': 'text-danger'};
            romeon = romeon.status;
            golfie['style'] = romeon;
            sizing = _closure1_slot9;
            output = golfie;
            romeon = copyDataProperties(output, sizing);
            sizing = _closure1_slot10;
            output = golfie;
            yankee = copyDataProperties(output, sizing);
            romeon = true;
            yankee = 'animated';
            golfie[yankee] = romeon;
            yankee = offset.message;
            offset = 'children';
            golfie[offset] = yankee;
            golfie = verify.bind(report)(option, golfie);
            oscard[1] = golfie;
            michal['children'] = oscard;
            entity = tangon.bind(report)(zuuluu, michal);
 702:
            return entity;
        }
    };
    var _closure1_slot11 = tangon;
    tangon = 13;
    tangon = oscard[tangon];
    oscard = report.bind(entity)(tangon);
    report = oscard.fileFinishedImporting;
    tangon = 'modules/auth/native/components/RegisterUsernameInput.tsx';
    tangon = report.bind(oscard)(tangon);
    michal = function(argFoo) { // Original name: RegisterUsernameInput
        _fun00003: for(var _fun00004_ip = 0; ; ) switch(_fun00004_ip) {
 0:
            zuuluu = argFoo;
            kiloes = zuuluu.username;
            michal = zuuluu.setUsername;
            var _closure2_slot0 = michal;
            offset = zuuluu.usernameStatus;
            backup = zuuluu.onSubmitEditing;
            report = _closure1_slot4;
            zuuluu = report.useState;
            yankee = true;
            option = zuuluu.bind(report)(yankee);
            golfie = _closure1_slot3;
            tangon = undefined;
            zuuluu = 2;
            golfie = golfie.bind(tangon)(option, zuuluu);
            zuuluu = 0;
            verify = golfie[zuuluu];
            zuuluu = 1;
            golfie = golfie[zuuluu];
            var _closure2_slot1 = golfie;
            option = report.useCallback;
            zuuluu = new Array(1);
            zuuluu[0] = michal;
            michal = function(argFoo) {
                michal = argFoo;
                zuuluu = _closure2_slot0;
                entity = michal.toLowerCase;
                michal = entity.bind(michal)();
                entity = undefined;
                michal = zuuluu.bind(entity)(michal);
                return entity;
            };
            sizing = option.bind(report)(michal, zuuluu);
            option = report.useCallback;
            zuuluu = new Array(1);
            zuuluu[0] = golfie;
            michal = function() {
                zuuluu = _closure2_slot1;
                entity = undefined;
                michal = true;
                michal = zuuluu.bind(entity)(michal);
                return entity;
            };
            foxtra = option.bind(report)(michal, zuuluu);
            zuuluu = report.useCallback;
            michal = new Array(1);
            michal[0] = golfie;
            entity = function() {
                zuuluu = _closure2_slot1;
                entity = undefined;
                michal = false;
                michal = zuuluu.bind(entity)(michal);
                return entity;
            };
            romeon = zuuluu.bind(report)(entity, michal);
            zuuluu = _closure1_slot6;
            michal = _closure1_slot7;
            entity = {};
            option = _closure1_slot5;
            output = _closure1_slot0;
            result = _closure1_slot2;
            report = 11;
            report = result[report];
            report = output.bind(tangon)(report);
            golfie = report.TextInput;
            report = {};
            echoed = 10;
            update = result[echoed];
            update = output.bind(tangon)(update);
            source = update.intl;
            update = source.string;
            echoed = result[echoed];
            echoed = output.bind(tangon)(echoed);
            echoed = echoed.t;
            echoed = echoed.IEpCBQ;
            echoed = update.bind(source)(echoed);
            report['label'] = echoed;
            report['onChange'] = sizing;
            sizing = false;
            report['autoCorrect'] = sizing;
            sizing = 12;
            echoed = result[sizing];
            update = output.bind(tangon)(echoed);
            echoed = update.isAndroid;
            echoed = echoed.bind(update)();
            report['secureTextEntry'] = echoed;
            sizing = result[sizing];
            output = output.bind(tangon)(sizing);
            sizing = output.isAndroid;
            output = sizing.bind(output)();
            sizing = 'default';
            if(!output) { _fun00004_ip = 323; continue _fun00003 }
 317:
            sizing = 'visible-password';
 323:
            report['keyboardType'] = sizing;
            report['value'] = kiloes;
            report['onSubmitEditing'] = backup;
            backup = 'next';
            report['returnKeyType'] = backup;
            backup = 'username';
            report['autoComplete'] = backup;
            report['textContentType'] = backup;
            backup = 'none';
            report['autoCapitalize'] = backup;
            report['onFocus'] = foxtra;
            report['onBlur'] = romeon;
            report['isClearable'] = yankee;
            report['autoFocus'] = yankee;
            yankee = null;
            yankee = yankee == offset;
            foxtra = undefined;
            if(yankee) { _fun00004_ip = 405; continue _fun00003 }
 400:
            foxtra = offset.type;
 405:
            romeon = _closure1_slot0;
            backup = _closure1_slot2;
            yankee = 5;
            yankee = backup[yankee];
            yankee = romeon.bind(tangon)(yankee);
            yankee = yankee.NameValidationState;
            romeon = yankee.ERROR;
            yankee = undefined;
            if(!(foxtra === romeon)) { _fun00004_ip = 447; continue _fun00003 }
 443:
            yankee = 'error';
 447:
            report['status'] = yankee;
            golfie = option.bind(tangon)(golfie, report);
            report = new Array(2);
            report[0] = golfie;
            option = _closure1_slot5;
            golfie = _closure1_slot11;
            oscard = {};
            oscard['usernameStatus'] = offset;
            oscard['isUsernameFocused'] = verify;
            oscard = option.bind(tangon)(golfie, oscard);
            report[1] = oscard;
            entity['children'] = report;
            entity = zuuluu.bind(tangon)(michal, entity);
            return entity;
        }
    };
    zuuluu['RegisterUsernameInput'] = michal;
    return entity;
})();