// app/modules/guild_scheduled_events/native/components/DeleteEventActionSheet.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    golf = argBaz;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = golf;
    var _closure1_slot2 = oscar;
    entity = global;
    verify = entity.Object;
    options = verify.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = options.bind(verify)(zulu, entity, tango);
    entity = 0;
    tango = oscar[entity];
    entity = undefined;
    tango = golf.bind(entity)(tango);
    var _closure1_slot3 = tango;
    tango = 1;
    options = oscar[tango];
    tango = argCorge;
    tango = tango.bind(entity)(options);
    var _closure1_slot4 = tango;
    tango = 2;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.View;
    var _closure1_slot5 = tango;
    tango = 3;
    tango = oscar[tango];
    tango = golf.bind(entity)(tango);
    var _closure1_slot6 = tango;
    tango = 4;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.DELETE_EVENT_SHEET_KEY;
    var _closure1_slot7 = tango;
    tango = 5;
    golf = oscar[tango];
    golf = report.bind(entity)(golf);
    golf = golf.jsx;
    var _closure1_slot8 = golf;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsxs;
    var _closure1_slot9 = tango;
    tango = 6;
    tango = oscar[tango];
    options = report.bind(entity)(tango);
    golf = options.createStyles;
    tango = {};
    yankee = 24;
    verify = {'paddingVertical': 24, 'paddingHorizontal': 16, 'alignItems': 'center'};
    tango['container'] = verify;
    offset = 8;
    verify = {'marginTop': 8, 'textAlign': 'center'};
    tango['subtitle'] = verify;
    verify = {};
    verify['marginTop'] = yankee;
    tango['cancelButton'] = verify;
    verify = {};
    verify['marginTop'] = offset;
    tango['confirmButton'] = verify;
    tango = golf.bind(options)(tango);
    var _closure1_slot10 = tango;
    tango = 14;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/guild_scheduled_events/native/components/DeleteEventActionSheet.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: DeleteEventActionSheet
        _fun102887: for(var _fun102887_ip = 0; ; ) switch(_fun102887_ip) {
 0:
            entity = argFoo;
            mike = entity.eventId;
            var _closure2_slot0 = mike;
            mike = entity.guildId;
            var _closure2_slot1 = mike;
            mike = entity.recurrenceId;
            var _closure2_slot2 = mike;
            entity = entity.eventException;
            var _closure2_slot3 = entity;
            tango = undefined;
            var _closure2_slot5 = tango;
            control = function() { // Original name: handleClose
                tango = _closure1_slot1;
                zulu = _closure1_slot2;
                entity = 8;
                zulu = zulu[entity];
                entity = undefined;
                tango = tango.bind(entity)(zulu);
                zulu = tango.hideActionSheet;
                mike = _closure1_slot7;
                mike = zulu.bind(tango)(mike);
                return entity;
            };
            var _closure2_slot6 = control;
            entity = _closure1_slot10;
            source = entity.bind(tango)();
            report = _closure1_slot4;
            zulu = report.useState;
            entity = false;
            report = zulu.bind(report)(entity);
            zulu = _closure1_slot3;
            entity = 2;
            zulu = zulu.bind(tango)(report, entity);
            entity = 0;
            foxtrot = zulu[entity];
            entity = 1;
            entity = zulu[entity];
            var _closure2_slot4 = entity;
            zulu = _closure1_slot0;
            report = _closure1_slot2;
            entity = 7;
            entity = report[entity];
            oscar = zulu.bind(tango)(entity);
            report = oscar.useStateFromStores;
            entity = _closure1_slot6;
            zulu = new Array(1);
            zulu[0] = entity;
            entity = function() {
                zulu = _closure1_slot6;
                mike = zulu.getGuildScheduledEvent;
                entity = _closure2_slot0;
                entity = mike.bind(zulu)(entity);
                return entity;
            };
            report = report.bind(oscar)(zulu, entity);
            entity = null;
            oscar = entity == report;
            zulu = undefined;
            if(oscar) { _fun102887_ip = 185; continue _fun102887 }
 179:
            zulu = report.recurrence_rule;
 185:
            sizing = entity != zulu;
            backup = entity != mike;
            _closure2_slot5 = backup;
            zulu = _closure1_slot8;
            mike = _closure1_slot1;
            options = _closure1_slot2;
            entity = 9;
            entity = options[entity];
            mike = mike.bind(tango)(entity);
            entity = {};
            golf = _closure1_slot9;
            oscar = _closure1_slot5;
            report = {};
            verify = source.container;
            report['style'] = verify;
            verify = _closure1_slot0;
            offset = 10;
            options = options[offset];
            options = verify.bind(tango)(options);
            verify = options.Text;
            options = {'variant': 'heading-xl/bold', 'color': 'header-primary'};
            output = _closure1_slot0;
            yankee = _closure1_slot2;
            echo = 11;
            result = yankee[echo];
            result = output.bind(tango)(result);
            update = result.intl;
            result = update.string;
            yankee = yankee[echo];
            yankee = output.bind(tango)(yankee);
            output = yankee.t;
            if(backup) { _fun102887_ip = 353; continue _fun102887 }
 324:
            if(sizing) { _fun102887_ip = 340; continue _fun102887 }
 327:
            yankee = output.B9sJLS;
            yankee = result.bind(update)(yankee);
            _fun102887_ip = 351; continue _fun102887;
 340:
            vacuum = output.wr33ra;
            yankee = result.bind(update)(vacuum);
 351:
            _fun102887_ip = 364; continue _fun102887;
 353:
            output = output.tqCll5;
            yankee = result.bind(update)(output);
 364:
            options['children'] = yankee;
            verify = zulu.bind(tango)(verify, options);
            options = new Array(5);
            options[0] = verify;
            output = _closure1_slot8;
            sequence = _closure1_slot0;
            result = _closure1_slot2;
            verify = result[offset];
            verify = sequence.bind(tango)(verify);
            yankee = verify.Text;
            verify = {'style': null, 'variant': 'text-md/medium', 'color': 'header-secondary'};
            update = source.subtitle;
            verify['style'] = update;
            update = result[echo];
            update = sequence.bind(tango)(update);
            vacuum = update.intl;
            update = vacuum.string;
            result = result[echo];
            result = sequence.bind(tango)(result);
            result = result.t;
            result = result.v2GWNT;
            result = update.bind(vacuum)(result);
            verify['children'] = result;
            verify = output.bind(tango)(yankee, verify);
            options[1] = verify;
            verify = sizing;
            if(!verify) { _fun102887_ip = 501; continue _fun102887 }
 498:
            verify = !backup;
 501:
            if(!verify) { _fun102887_ip = 613; continue _fun102887 }
 504:
            output = _closure1_slot8;
            update = _closure1_slot0;
            result = _closure1_slot2;
            offset = result[offset];
            offset = update.bind(tango)(offset);
            yankee = offset.Text;
            offset = {'style': null, 'variant': 'text-md/medium', 'color': 'header-secondary'};
            vacuum = source.subtitle;
            offset['style'] = vacuum;
            vacuum = result[echo];
            vacuum = update.bind(tango)(vacuum);
            sequence = vacuum.intl;
            vacuum = sequence.format;
            result = result[echo];
            result = update.bind(tango)(result);
            result = result.t;
            update = result.ZcpcyM;
            result = {};
            result = vacuum.bind(sequence)(update, result);
            offset['children'] = result;
            verify = output.bind(tango)(yankee, offset);
 613:
            options[2] = verify;
            yankee = _closure1_slot8;
            result = _closure1_slot1;
            update = _closure1_slot2;
            output = 12;
            verify = update[output];
            offset = result.bind(tango)(verify);
            verify = {};
            vacuum = source.cancelButton;
            verify['style'] = vacuum;
            vacuum = update[output];
            vacuum = result.bind(tango)(vacuum);
            vacuum = vacuum.Colors;
            vacuum = vacuum.GREY;
            verify['color'] = vacuum;
            record = _closure1_slot0;
            vacuum = update[echo];
            vacuum = record.bind(tango)(vacuum);
            config = vacuum.intl;
            sequence = config.string;
            vacuum = update[echo];
            vacuum = record.bind(tango)(vacuum);
            vacuum = vacuum.t;
            vacuum = vacuum.oEAioK;
            vacuum = sequence.bind(config)(vacuum);
            verify['text'] = vacuum;
            verify['onPress'] = control;
            verify = yankee.bind(tango)(offset, verify);
            options[3] = verify;
            verify = update[output];
            offset = result.bind(tango)(verify);
            verify = {};
            source = source.confirmButton;
            verify['style'] = source;
            output = update[output];
            output = result.bind(tango)(output);
            output = output.Colors;
            output = output.RED;
            verify['color'] = output;
            if(!sizing) { _fun102887_ip = 850; continue _fun102887 }
 793:
            if(backup) { _fun102887_ip = 850; continue _fun102887 }
 796:
            result = _closure1_slot0;
            backup = _closure1_slot2;
            sizing = backup[echo];
            sizing = result.bind(tango)(sizing);
            output = sizing.intl;
            sizing = output.string;
            backup = backup[echo];
            backup = result.bind(tango)(backup);
            backup = backup.t;
            backup = backup.8ZsNv7;
            backup = sizing.bind(output)(backup);
            _fun102887_ip = 902; continue _fun102887;
 850:
            result = _closure1_slot0;
            kilo = _closure1_slot2;
            sizing = kilo[echo];
            sizing = result.bind(tango)(sizing);
            output = sizing.intl;
            sizing = output.string;
            kilo = kilo[echo];
            kilo = result.bind(tango)(kilo);
            kilo = kilo.t;
            kilo = kilo.B9sJLS;
            backup = sizing.bind(output)(kilo);
 902:
            verify['text'] = backup;
            verify['loading'] = foxtrot;
            romeo = function() { // Original name: onPress
                _fun102890: for(var _fun102890_ip = 0; ; ) switch(_fun102890_ip) {
 0:
                    tango = _closure2_slot4;
                    entity = undefined;
                    zulu = true;
                    zulu = tango.bind(entity)(zulu);
                    zulu = _closure2_slot5;
                    report = _closure1_slot1;
                    oscar = _closure1_slot2;
                    tango = 13;
                    tango = oscar[tango];
                    options = report.bind(entity)(tango);
                    if(zulu) { _fun102890_ip = 68; continue _fun102890 }
 46:
                    report = options.deleteGuildEvent;
                    tango = _closure2_slot0;
                    zulu = _closure2_slot1;
                    zulu = report.bind(options)(tango, zulu);
                    _fun102890_ip = 97; continue _fun102890;
 68:
                    golf = options.deleteRecurrence;
                    romeo = _closure2_slot1;
                    yankee = _closure2_slot0;
                    offset = _closure2_slot2;
                    verify = _closure2_slot3;
                    foxtrot = options;
                    zulu = foxtrot[golf](romeo, yankee, offset, verify, options);
 97:
                    mike = _closure2_slot6;
                    mike = mike.bind(entity)();
                    return entity;
                }
            };
            verify['onPress'] = romeo;
            verify = yankee.bind(tango)(offset, verify);
            options[4] = verify;
            report['children'] = options;
            report = golf.bind(tango)(oscar, report);
            entity['children'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();