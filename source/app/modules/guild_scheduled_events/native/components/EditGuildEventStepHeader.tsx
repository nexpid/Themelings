// app/modules/guild_scheduled_events/native/components/EditGuildEventStepHeader.tsx
export default (function(argFoo, argBar, _, argCor, __, argFre, argPlu) {
    report = argBar;
    zuuluu = argFre;
    oscard = argPlu;
    var _closure1_slot0 = report;
    var _closure1_slot1 = oscard;
    entity = global;
    option = entity.Object;
    golfie = option.defineProperty;
    tangon = {};
    entity = true;
    tangon['value'] = entity;
    entity = '__esModule';
    entity = golfie.bind(option)(zuuluu, entity, tangon);
    entity = 0;
    golfie = oscard[entity];
    tangon = argCor;
    entity = undefined;
    tangon = tangon.bind(entity)(golfie);
    tangon = 1;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    tangon = tangon.View;
    var _closure1_slot2 = tangon;
    tangon = 2;
    golfie = oscard[tangon];
    golfie = report.bind(entity)(golfie);
    golfie = golfie.jsx;
    var _closure1_slot3 = golfie;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    tangon = tangon.jsxs;
    var _closure1_slot4 = tangon;
    tangon = 3;
    tangon = oscard[tangon];
    option = report.bind(entity)(tangon);
    golfie = option.createStyles;
    tangon = {};
    offset = 'center';
    verify = {'alignItems': 'center', 'paddingBottom': 24};
    tangon['header'] = verify;
    verify = {'marginTop': 8, 'marginBottom': 8};
    tangon['headerTitle'] = verify;
    verify = {};
    verify['textAlign'] = offset;
    tangon['headerSubtitle'] = verify;
    tangon = golfie.bind(option)(tangon);
    var _closure1_slot5 = tangon;
    tangon = 5;
    tangon = oscard[tangon];
    oscard = report.bind(entity)(tangon);
    report = oscard.fileFinishedImporting;
    tangon = 'modules/guild_scheduled_events/native/components/EditGuildEventStepHeader.tsx';
    tangon = report.bind(oscard)(tangon);
    michal = function(argFoo) { // Original name: EditGuildEventStepHeader
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            entity = argFoo;
            verify = entity.title;
            offset = entity.subtitle;
            entity = _closure1_slot5;
            tangon = undefined;
            yankee = entity.bind(tangon)();
            zuuluu = _closure1_slot4;
            michal = _closure1_slot2;
            entity = {};
            report = yankee.header;
            entity['style'] = report;
            option = _closure1_slot3;
            oscard = _closure1_slot0;
            report = _closure1_slot1;
            romeon = 4;
            report = report[romeon];
            report = oscard.bind(tangon)(report);
            oscard = report.Text;
            report = {'style': null, 'accessibilityRole': 'header', 'variant': 'heading-xl/semibold', 'color': 'header-primary'};
            foxtra = yankee.headerTitle;
            report['style'] = foxtra;
            report['children'] = verify;
            oscard = option.bind(tangon)(oscard, report);
            report = new Array(2);
            report[0] = oscard;
            verify = null;
            option = verify != offset;
            oscard = null;
            if(!option) { _fun00002_ip = 202; continue _fun00001 }
 130:
            option = '';
            oscard = null;
            if(!(option !== offset)) { _fun00002_ip = 202; continue _fun00001 }
 140:
            verify = _closure1_slot3;
            option = _closure1_slot0;
            golfie = _closure1_slot1;
            golfie = golfie[romeon];
            golfie = option.bind(tangon)(golfie);
            option = golfie.Text;
            golfie = {'style': null, 'variant': 'text-sm/medium', 'color': 'header-secondary'};
            yankee = yankee.headerSubtitle;
            golfie['style'] = yankee;
            golfie['children'] = offset;
            oscard = verify.bind(tangon)(option, golfie);
 202:
            report[1] = oscard;
            entity['children'] = report;
            entity = zuuluu.bind(tangon)(michal, entity);
            return entity;
        }
    };
    zuuluu['default'] = michal;
    return entity;
})();