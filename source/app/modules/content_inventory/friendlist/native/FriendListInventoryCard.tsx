// app/modules/content_inventory/friendlist/native/FriendListInventoryCard.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    oscar = argBar;
    romeo = argBaz;
    zulu = argFred;
    golf = argPlugh;
    var _closure1_slot0 = oscar;
    var _closure1_slot1 = romeo;
    var _closure1_slot2 = golf;
    entity = global;
    options = entity.Object;
    report = options.defineProperty;
    mike = {};
    entity = true;
    mike['value'] = entity;
    entity = '__esModule';
    entity = report.bind(options)(zulu, entity, mike);
    entity = 0;
    mike = golf[entity];
    entity = undefined;
    mike = romeo.bind(entity)(mike);
    var _closure1_slot3 = mike;
    mike = 1;
    mike = golf[mike];
    mike = romeo.bind(entity)(mike);
    var _closure1_slot4 = mike;
    foxtrot = 2;
    report = golf[foxtrot];
    mike = argCorge;
    mike = mike.bind(entity)(report);
    var _closure1_slot5 = mike;
    mike = 3;
    mike = golf[mike];
    mike = oscar.bind(entity)(mike);
    mike = mike.View;
    var _closure1_slot6 = mike;
    mike = 4;
    report = golf[mike];
    report = oscar.bind(entity)(report);
    report = report.jsx;
    var _closure1_slot7 = report;
    report = golf[mike];
    report = oscar.bind(entity)(report);
    report = report.jsxs;
    var _closure1_slot8 = report;
    mike = golf[mike];
    mike = oscar.bind(entity)(mike);
    mike = mike.Fragment;
    var _closure1_slot9 = mike;
    mike = 5;
    mike = golf[mike];
    verify = oscar.bind(entity)(mike);
    options = verify.createStyles;
    report = {};
    mike = 96;
    offset = {'width': 96, 'display': 'flex', 'flexDirection': 'column', 'justifyContent': 'center', 'alignItems': 'center', 'marginLeft': 12};
    report['card'] = offset;
    offset = {'height': 96, 'width': 96, 'borderRadius': null, 'display': 'flex', 'flexDirection': 'column', 'alignItems': 'center', 'justifyContent': 'center'};
    yankee = 6;
    backup = golf[yankee];
    backup = romeo.bind(entity)(backup);
    backup = backup.radii;
    backup = backup.lg;
    offset['borderRadius'] = backup;
    report['gameImageContainer'] = offset;
    offset = {};
    backup = golf[yankee];
    backup = romeo.bind(entity)(backup);
    backup = backup.colors;
    backup = backup.STATUS_POSITIVE;
    offset['borderColor'] = backup;
    offset['borderWidth'] = foxtrot;
    report['live'] = offset;
    offset = {'height': 96, 'width': 96};
    foxtrot = golf[yankee];
    foxtrot = romeo.bind(entity)(foxtrot);
    foxtrot = foxtrot.radii;
    foxtrot = foxtrot.lg;
    offset['borderRadius'] = foxtrot;
    report['gameImage'] = offset;
    offset = {'borderColor': 'transparent', 'borderWidth': 2, 'height': 88, 'width': 88};
    report['liveGame'] = offset;
    offset = {'position': 'absolute', 'bottom': 28, 'left': 4294967290, 'borderColor': null, 'borderWidth': 6, 'borderRadius': null, 'height': 44, 'width': 44};
    foxtrot = golf[yankee];
    foxtrot = romeo.bind(entity)(foxtrot);
    foxtrot = foxtrot.colors;
    foxtrot = foxtrot.BACKGROUND_MOBILE_PRIMARY;
    offset['borderColor'] = foxtrot;
    yankee = golf[yankee];
    yankee = romeo.bind(entity)(yankee);
    yankee = yankee.radii;
    yankee = yankee.round;
    offset['borderRadius'] = yankee;
    report['avatar'] = offset;
    offset = {'display': 'flex', 'flexDirection': 'column'};
    report['col'] = offset;
    offset = {'display': 'flex', 'flexDirection': 'row'};
    report['row'] = offset;
    report = options.bind(verify)(report);
    var _closure1_slot10 = report;
    report = 19;
    report = golf[report];
    golf = oscar.bind(entity)(report);
    oscar = golf.fileFinishedImporting;
    report = 'modules/content_inventory/friendlist/native/FriendListInventoryCard.tsx';
    report = oscar.bind(golf)(report);
    tango = function(argFoo) { // Original name: _default
        _fun115055: for(var _fun115055_ip = 0; ; ) switch(_fun115055_ip) {
 0:
            entity = argFoo;
            zulu = entity.content;
            var _closure2_slot0 = zulu;
            foxtrot = entity.index;
            tango = undefined;
            var _closure2_slot2 = tango;
            entity = _closure1_slot10;
            vacuum = entity.bind(tango)();
            mike = _closure1_slot5;
            entity = mike.useState;
            mike = entity.bind(mike)();
            entity = _closure1_slot4;
            yankee = 2;
            entity = entity.bind(tango)(mike, yankee);
            oscar = 0;
            control = entity[oscar];
            report = 1;
            entity = entity[report];
            var _closure2_slot1 = entity;
            mike = _closure1_slot0;
            golf = _closure1_slot2;
            entity = 7;
            entity = golf[entity];
            golf = mike.bind(tango)(entity);
            mike = golf.useGetOrFetchApplication;
            entity = zulu.extra;
            entity = entity.application_id;
            sizing = mike.bind(golf)(entity);
            source = null;
            mike = source == sizing;
            entity = undefined;
            if(mike) { _fun115055_ip = 154; continue _fun115055 }
 133:
            options = sizing.getIconURL;
            golf = 128;
            mike = 'png';
            entity = options.bind(sizing)(golf, mike);
 154:
            mike = source != entity;
            echo = '';
            result = echo;
            if(!mike) { _fun115055_ip = 171; continue _fun115055 }
 168:
            result = entity;
 171:
            mike = _closure1_slot0;
            verify = _closure1_slot2;
            entity = 8;
            entity = verify[entity];
            options = mike.bind(tango)(entity);
            golf = options.useAvatarColors;
            entity = 'black';
            entity = golf.bind(options)(result, entity);
            options = _closure1_slot4;
            entity = options.bind(tango)(entity, yankee);
            golf = entity[oscar];
            update = entity[report];
            golf = _closure1_slot1;
            entity = 9;
            entity = verify[entity];
            backup = golf.bind(tango)(entity);
            entity = function() {
                mike = function() {
                    tango = _closure1_slot3;
                    zulu = undefined;
                    mike = function* () {
                        entity = function* () { // Original name: ?anon_0_
                            _fun115059: for(var _fun115059_ip = 0; ; ) switch(_fun115059_ip) {
 0:
                                StartGenerator();
                                ResumeGenerator(result_out_reg=0, return_bool_out_reg=1);
                                if(mike) { _fun115059_ip = 81; continue _fun115059 }
 7:
                                tango = _closure1_slot0;
                                zulu = _closure1_slot2;
                                mike = 10;
                                mike = zulu[mike];
                                zulu = undefined;
                                oscar = tango.bind(zulu)(mike);
                                report = oscar.getUser;
                                mike = _closure2_slot0;
                                mike = mike.author_id;
                                mike = report.bind(oscar)(mike);
                                SaveGenerator(address=60);
 58:
                                return mike;
 60:
                                ResumeGenerator(result_out_reg=1, return_bool_out_reg=4);
                                if(report) { _fun115059_ip = 78; continue _fun115059 }
 66:
                                tango = _closure2_slot1;
                                tango = tango.bind(zulu)(mike);
                                return zulu;
 78:
                                return mike;
 81:
                                return entity;
                            }
                        };
                        return entity;
                    };
                    mike = tango.bind(zulu)(mike);
                    var _closure4_slot0 = mike;
                    entity = function() {
                        entity = undefined;
                        tango = _closure4_slot0;
                        zulu = tango.apply;
                        entity = arguments;
                        mike = entity;
                        entity = this;
                        entity = zulu.bind(tango)(entity, mike);
                        return entity;
                    };
                    return entity;
                };
                entity = undefined;
                mike = mike.bind(entity)();
                mike = mike.bind(entity)();
                return entity;
            };
            entity = backup.bind(tango)(entity);
            kilo = _closure1_slot5;
            backup = kilo.useState;
            entity = false;
            entity = backup.bind(kilo)(entity);
            entity = options.bind(tango)(entity, yankee);
            oscar = entity[oscar];
            entity = entity[report];
            _closure2_slot2 = entity;
            entity = 11;
            entity = verify[entity];
            mike = mike.bind(tango)(entity);
            entity = mike.isEntryActive;
            config = entity.bind(mike)(zulu);
            zulu = _closure1_slot8;
            mike = _closure1_slot9;
            entity = {};
            options = _closure1_slot7;
            report = 12;
            report = verify[report];
            golf = golf.bind(tango)(report);
            report = {};
            verify = function() { // Original name: onPress
                zulu = _closure2_slot2;
                entity = undefined;
                mike = true;
                mike = zulu.bind(entity)(mike);
                return entity;
            };
            report['onPress'] = verify;
            yankee = _closure1_slot6;
            verify = {};
            backup = vacuum.card;
            verify['style'] = backup;
            backup = {};
            kilo = vacuum.col;
            backup['style'] = kilo;
            kilo = {};
            sequence = vacuum.gameImageContainer;
            output = new Array(3);
            output[0] = sequence;
            record = result;
            result = config;
            if(!config) { _fun115055_ip = 408; continue _fun115055 }
 402:
            result = vacuum.live;
 408:
            output[1] = result;
            result = {};
            result['backgroundColor'] = update;
            output[2] = result;
            kilo['style'] = output;
            sizing = source != sizing;
            if(!sizing) { _fun115055_ip = 506; continue _fun115055 }
 433:
            update = _closure1_slot7;
            result = _closure1_slot1;
            sequence = _closure1_slot2;
            output = 13;
            output = sequence[output];
            result = result.bind(tango)(output);
            output = {};
            sequence = {};
            sequence['uri'] = record;
            output['source'] = sequence;
            record = vacuum.gameImage;
            sequence = new Array(2);
            sequence[0] = record;
            if(!config) { _fun115055_ip = 492; continue _fun115055 }
 486:
            config = vacuum.liveGame;
 492:
            sequence[1] = config;
            output['style'] = sequence;
            sizing = update.bind(tango)(result, output);
 506:
            kilo['children'] = sizing;
            sizing = options.bind(tango)(yankee, kilo);
            kilo = new Array(3);
            kilo[0] = sizing;
            sizing = source != control;
            if(!sizing) { _fun115055_ip = 629; continue _fun115055 }
 531:
            update = _closure1_slot7;
            result = _closure1_slot6;
            output = {};
            vacuum = vacuum.avatar;
            output['style'] = vacuum;
            sequence = _closure1_slot1;
            context = _closure1_slot2;
            vacuum = 14;
            vacuum = context[vacuum];
            sequence = sequence.bind(tango)(vacuum);
            vacuum = {};
            record = _closure1_slot0;
            config = 15;
            config = context[config];
            config = record.bind(tango)(config);
            config = config.AvatarSizes;
            config = config.REFRESH_MEDIUM_32;
            vacuum['size'] = config;
            vacuum['user'] = control;
            vacuum['guildId'] = tango;
            vacuum = update.bind(tango)(sequence, vacuum);
            output['children'] = vacuum;
            sizing = update.bind(tango)(result, output);
 629:
            kilo[1] = sizing;
            result = _closure1_slot7;
            output = _closure1_slot0;
            update = _closure1_slot2;
            sizing = 16;
            sizing = update[sizing];
            sizing = output.bind(tango)(sizing);
            output = sizing.Text;
            sizing = {};
            update = 'text-sm/normal';
            sizing['variant'] = update;
            update = {'textAlign': 'center', 'marginTop': 12};
            sizing['style'] = update;
            vacuum = source == control;
            update = undefined;
            if(vacuum) { _fun115055_ip = 703; continue _fun115055 }
 697:
            update = control.globalName;
 703:
            source = source != update;
            if(!source) { _fun115055_ip = 713; continue _fun115055 }
 710:
            echo = update;
 713:
            sizing['children'] = echo;
            sizing = result.bind(tango)(output, sizing);
            kilo[2] = sizing;
            backup['children'] = kilo;
            backup = zulu.bind(tango)(yankee, backup);
            verify['children'] = backup;
            verify = options.bind(tango)(yankee, verify);
            report['children'] = verify;
            golf = options.bind(tango)(golf, report);
            report = new Array(2);
            report[0] = golf;
            if(!oscar) { _fun115055_ip = 851; continue _fun115055 }
 768:
            verify = _closure1_slot7;
            options = _closure1_slot0;
            backup = _closure1_slot2;
            golf = 17;
            golf = backup[golf];
            golf = options.bind(tango)(golf);
            options = golf.Portal;
            golf = {};
            yankee = _closure1_slot1;
            offset = 18;
            offset = backup[offset];
            yankee = yankee.bind(tango)(offset);
            offset = {};
            offset['focusedIndex'] = foxtrot;
            romeo = function() { // Original name: onDismiss
                zulu = _closure2_slot2;
                mike = undefined;
                entity = false;
                entity = zulu.bind(mike)(entity);
                return entity;
            };
            offset['onDismiss'] = romeo;
            offset = verify.bind(tango)(yankee, offset);
            golf['children'] = offset;
            oscar = verify.bind(tango)(options, golf);
 851:
            report[1] = oscar;
            entity['children'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    zulu['default'] = tango;
    zulu['TILE_WIDTH'] = mike;
    mike = 128;
    zulu['TILE_HEIGHT'] = mike;
    return entity;
})();