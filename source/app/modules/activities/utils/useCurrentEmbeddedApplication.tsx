// app/modules/activities/utils/useCurrentEmbeddedApplication.tsx
export default (function(_, argBar, argBaz, __, ___, argFred, argPlugh) {
    oscar = argBaz;
    zulu = argFred;
    report = argPlugh;
    var _closure1_slot0 = oscar;
    var _closure1_slot1 = report;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    tango = report[entity];
    entity = undefined;
    tango = oscar.bind(entity)(tango);
    var _closure1_slot2 = tango;
    tango = 3;
    report = report[tango];
    tango = argBar;
    oscar = tango.bind(entity)(report);
    report = oscar.fileFinishedImporting;
    tango = 'modules/activities/utils/useCurrentEmbeddedApplication.tsx';
    tango = report.bind(oscar)(tango);
    mike = function() { // Original name: useCurrentEmbeddedApplication
        _fun75864: for(var _fun75864_ip = 0; ; ) switch(_fun75864_ip) {
 0:
            mike = arguments[0];
            entity = undefined;
            if(!(mike === entity)) { _fun75864_ip = 11; continue _fun75864 }
 9:
            mike = {};
 11:
            golf = mike.fetchesApplication;
            if(!(golf === entity)) { _fun75864_ip = 23; continue _fun75864 }
 21:
            golf = true;
 23:
            oscar = _closure1_slot0;
            options = _closure1_slot1;
            report = 1;
            zulu = options[report];
            zulu = oscar.bind(entity)(zulu);
            tango = zulu.bind(entity)();
            zulu = 2;
            zulu = options[zulu];
            oscar = oscar.bind(entity)(zulu);
            zulu = null;
            if(!(zulu != tango)) { _fun75864_ip = 83; continue _fun75864 }
 68:
            options = tango.applicationId;
            tango = new Array(1);
            tango[0] = options;
            _fun75864_ip = 87; continue _fun75864;
 83:
            tango = new Array(0);
 87:
            tango = oscar.bind(entity)(tango, golf);
            mike = _closure1_slot2;
            tango = mike.bind(entity)(tango, report);
            mike = 0;
            mike = tango[mike];
            zulu = zulu != mike;
            entity = undefined;
            if(!zulu) { _fun75864_ip = 121; continue _fun75864 }
 118:
            entity = mike;
 121:
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();