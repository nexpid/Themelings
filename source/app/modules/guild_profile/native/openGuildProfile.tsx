// app/modules/guild_profile/native/openGuildProfile.tsx
export default (function(argFoo, argBar, argBaz, _, __, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    entity = argBaz;
    var _closure1_slot1 = entity;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    tango = oscar[entity];
    entity = undefined;
    tango = report.bind(entity)(tango);
    golf = tango.AnalyticEvents;
    var _closure1_slot3 = golf;
    tango = tango.GuildFeatures;
    var _closure1_slot4 = tango;
    tango = 8;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/guild_profile/native/openGuildProfile.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: openGuildProfile
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            mike = argFoo;
            yankee = arguments[1];
            entity = undefined;
            if(!(yankee === entity)) { _fun00002_ip = 14; continue _fun00001 }
 12:
            yankee = true;
 14:
            report = _closure1_slot1;
            oscar = _closure1_slot2;
            zulu = 1;
            zulu = oscar[zulu];
            options = report.bind(entity)(zulu);
            golf = options.track;
            zulu = _closure1_slot3;
            report = zulu.OPEN_POPOUT;
            zulu = {};
            verify = 'Guild Profile';
            zulu['type'] = verify;
            verify = mike.id;
            zulu['guild_id'] = verify;
            zulu = golf.bind(options)(report, zulu);
            report = _closure1_slot0;
            zulu = 2;
            zulu = oscar[zulu];
            oscar = report.bind(entity)(zulu);
            report = oscar.shouldNSFWGateGuild;
            zulu = mike.id;
            zulu = report.bind(oscar)(zulu);
            if(zulu) { _fun00002_ip = 289; continue _fun00001 }
 117:
            report = mike.hasFeature;
            zulu = _closure1_slot4;
            zulu = zulu.HUB;
            report = report.bind(mike)(zulu);
            golf = _closure1_slot1;
            zulu = _closure1_slot2;
            oscar = 3;
            oscar = zulu[oscar];
            options = golf.bind(entity)(oscar);
            golf = options.openLazy;
            verify = _closure1_slot0;
            oscar = 5;
            oscar = zulu[oscar];
            oscar = verify.bind(entity)(oscar);
            if(report) { _fun00002_ip = 250; continue _fun00001 }
 183:
            report = 7;
            verify = zulu[report];
            report = zulu.paths;
            offset = oscar.bind(entity)(verify, report);
            romeo = mike.id;
            report = global;
            report = report.HermesInternal;
            verify = report.concat;
            report = 'GuildProfile:';
            verify = verify.bind(report)(romeo);
            report = {};
            report['guild'] = mike;
            report['expanded'] = yankee;
            report = golf.bind(options)(offset, verify, report);
            _fun00002_ip = 368; continue _fun00001;
 250:
            report = 6;
            report = zulu[report];
            zulu = zulu.paths;
            oscar = oscar.bind(entity)(report, zulu);
            report = {};
            report['guild'] = mike;
            zulu = 'GuildProfile';
            zulu = golf.bind(options)(oscar, zulu, report);
            _fun00002_ip = 368; continue _fun00001;
 289:
            oscar = _closure1_slot1;
            zulu = _closure1_slot2;
            report = 3;
            report = zulu[report];
            oscar = oscar.bind(entity)(report);
            report = oscar.openLazy;
            golf = _closure1_slot0;
            tango = 5;
            tango = zulu[tango];
            golf = golf.bind(entity)(tango);
            tango = 4;
            tango = zulu[tango];
            zulu = zulu.paths;
            tango = golf.bind(entity)(tango, zulu);
            zulu = {};
            zulu['guild'] = mike;
            mike = 'NsfwGateGuildSettingsActionSheet';
            mike = report.bind(oscar)(tango, mike, zulu);
 368:
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();