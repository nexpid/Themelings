// app/modules/forums/native/posts/ForumPostTitle.tsx
export default (function(argFoo, argBar, _, argCorge, __, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    golf = oscar[entity];
    tango = argCorge;
    entity = undefined;
    tango = tango.bind(entity)(golf);
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsx;
    var _closure1_slot2 = tango;
    tango = 2;
    tango = oscar[tango];
    options = report.bind(entity)(tango);
    golf = options.createStyles;
    tango = {};
    verify = {};
    offset = 6;
    verify['marginBottom'] = offset;
    tango['title'] = verify;
    tango = golf.bind(options)(tango);
    var _closure1_slot3 = tango;
    tango = 4;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/forums/native/posts/ForumPostTitle.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: ForumPostTitle
        _fun100541: for(var _fun100541_ip = 0; ; ) switch(_fun100541_ip) {
 0:
            entity = argFoo;
            report = entity.title;
            verify = entity.lineClamp;
            options = entity.ellipsizeMode;
            mike = entity.hasUnreads;
            oscar = entity.onTextLayout;
            zulu = _closure1_slot3;
            tango = undefined;
            golf = zulu.bind(tango)();
            offset = 'text-muted';
            if(!mike) { _fun100541_ip = 60; continue _fun100541 }
 54:
            offset = 'header-primary';
 60:
            zulu = _closure1_slot2;
            mike = _closure1_slot0;
            yankee = _closure1_slot1;
            entity = 3;
            entity = yankee[entity];
            entity = mike.bind(tango)(entity);
            mike = entity.Text;
            entity = {};
            yankee = 'text-md/semibold';
            entity['variant'] = yankee;
            entity['color'] = offset;
            entity['lineClamp'] = verify;
            entity['ellipsizeMode'] = options;
            golf = golf.title;
            entity['style'] = golf;
            entity['onTextLayout'] = oscar;
            entity['children'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();