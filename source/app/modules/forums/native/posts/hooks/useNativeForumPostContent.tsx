// app/modules/forums/native/posts/hooks/useNativeForumPostContent.tsx
export default (function(argFoo, argBar, argBaz, _, __, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    entity = argBaz;
    var _closure1_slot1 = entity;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    tango = oscar[entity];
    entity = undefined;
    tango = report.bind(entity)(tango);
    tango = tango.MessageFlags;
    var _closure1_slot3 = tango;
    tango = 1;
    tango = oscar[tango];
    options = report.bind(entity)(tango);
    golf = options.createStyles;
    tango = {};
    verify = {};
    offset = 'italic';
    verify['fontStyle'] = offset;
    tango['italics'] = verify;
    tango = golf.bind(options)(tango);
    var _closure1_slot4 = tango;
    tango = 5;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/forums/native/posts/hooks/useNativeForumPostContent.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: useNativeForumPostContent
        _fun100593: for(var _fun100593_ip = 0; ; ) switch(_fun100593_ip) {
 0:
            entity = argFoo;
            verify = entity.message;
            tango = entity.messageContent;
            report = entity.messageLoaded;
            entity = entity.isMessageDeleted;
            mike = _closure1_slot4;
            golf = undefined;
            mike = mike.bind(golf)();
            if(entity) { _fun100593_ip = 690; continue _fun100593 }
 45:
            entity = null;
            if(!(entity != verify)) { _fun100593_ip = 82; continue _fun100593 }
 51:
            options = _closure1_slot1;
            offset = _closure1_slot2;
            oscar = 3;
            oscar = offset[oscar];
            oscar = options.bind(golf)(oscar);
            oscar = oscar.bind(golf)(verify);
            if(oscar) { _fun100593_ip = 662; continue _fun100593 }
 82:
            if(!(entity != verify)) { _fun100593_ip = 98; continue _fun100593 }
 86:
            oscar = verify.blocked;
            if(oscar) { _fun100593_ip = 579; continue _fun100593 }
 98:
            options = entity == verify;
            oscar = undefined;
            if(options) { _fun100593_ip = 112; continue _fun100593 }
 107:
            oscar = verify.content;
 112:
            options = entity == oscar;
            if(options) { _fun100593_ip = 141; continue _fun100593 }
 119:
            oscar = entity == verify;
            offset = undefined;
            if(oscar) { _fun100593_ip = 133; continue _fun100593 }
 128:
            offset = verify.content;
 133:
            oscar = '';
            options = oscar === offset;
 141:
            if(options) { _fun100593_ip = 148; continue _fun100593 }
 144:
            options = entity == tango;
 148:
            if(options) { _fun100593_ip = 159; continue _fun100593 }
 151:
            oscar = '';
            options = oscar === tango;
 159:
            if(options) { _fun100593_ip = 197; continue _fun100593 }
 162:
            oscar = global;
            offset = oscar.Array;
            oscar = offset.isArray;
            oscar = oscar.bind(offset)(tango);
            if(!oscar) { _fun100593_ip = 194; continue _fun100593 }
 183:
            yankee = tango.length;
            offset = 0;
            oscar = offset === yankee;
 194:
            options = oscar;
 197:
            oscar = null;
            if(!report) { _fun100593_ip = 548; continue _fun100593 }
 205:
            if(!(entity != verify)) { _fun100593_ip = 490; continue _fun100593 }
 212:
            offset = _closure1_slot0;
            yankee = _closure1_slot2;
            report = 4;
            report = yankee[report];
            offset = offset.bind(golf)(report);
            report = offset.getMessageStickers;
            report = report.bind(offset)(verify);
            report = report.length;
            offset = 0;
            if(!(!(report > offset))) { _fun100593_ip = 433; continue _fun100593 }
 257:
            yankee = verify.hasFlag;
            report = _closure1_slot3;
            report = report.IS_VOICE_MESSAGE;
            report = yankee.bind(verify)(report);
            if(report) { _fun100593_ip = 376; continue _fun100593 }
 281:
            report = verify.embeds;
            report = report.length;
            report = report > offset;
            if(report) { _fun100593_ip = 314; continue _fun100593 }
 299:
            verify = verify.attachments;
            verify = verify.length;
            report = verify > offset;
 314:
            entity = null;
            if(!report) { _fun100593_ip = 374; continue _fun100593 }
 319:
            yankee = _closure1_slot0;
            romeo = _closure1_slot2;
            report = 2;
            verify = romeo[report];
            verify = yankee.bind(golf)(verify);
            offset = verify.intl;
            verify = offset.string;
            report = romeo[report];
            report = yankee.bind(golf)(report);
            report = report.t;
            report = report.JAKsMz;
            entity = verify.bind(offset)(report);
 374:
            _fun100593_ip = 431; continue _fun100593;
 376:
            yankee = _closure1_slot0;
            romeo = _closure1_slot2;
            report = 2;
            verify = romeo[report];
            verify = yankee.bind(golf)(verify);
            offset = verify.intl;
            verify = offset.string;
            report = romeo[report];
            report = yankee.bind(golf)(report);
            report = report.t;
            report = report.6bhHra;
            entity = verify.bind(offset)(report);
 431:
            _fun100593_ip = 488; continue _fun100593;
 433:
            yankee = _closure1_slot0;
            romeo = _closure1_slot2;
            report = 2;
            verify = romeo[report];
            verify = yankee.bind(golf)(verify);
            offset = verify.intl;
            verify = offset.string;
            report = romeo[report];
            report = yankee.bind(golf)(report);
            report = report.t;
            report = report.7K5LmZ;
            entity = verify.bind(offset)(report);
 488:
            _fun100593_ip = 545; continue _fun100593;
 490:
            yankee = _closure1_slot0;
            romeo = _closure1_slot2;
            report = 2;
            verify = romeo[report];
            verify = yankee.bind(golf)(verify);
            offset = verify.intl;
            verify = offset.string;
            report = romeo[report];
            report = yankee.bind(golf)(report);
            report = report.t;
            report = report.mE3KJC;
            entity = verify.bind(offset)(report);
 545:
            oscar = entity;
 548:
            entity = {'content': null, 'style': null, 'variant': 'text-sm/medium'};
            report = tango;
            if(!options) { _fun100593_ip = 571; continue _fun100593 }
 568:
            report = oscar;
 571:
            entity['content'] = report;
            return entity;
 579:
            entity = {};
            verify = _closure1_slot0;
            offset = _closure1_slot2;
            report = 2;
            oscar = offset[report];
            oscar = verify.bind(golf)(oscar);
            options = oscar.intl;
            oscar = options.string;
            report = offset[report];
            report = verify.bind(golf)(report);
            report = report.t;
            report = report.Lkp2fH;
            report = oscar.bind(options)(report);
            entity['content'] = report;
            report = mike.italics;
            entity['style'] = report;
            report = 'text-sm/normal';
            entity['variant'] = report;
            return entity;
 662:
            entity = {};
            entity['content'] = tango;
            tango = mike.italics;
            entity['style'] = tango;
            tango = 'text-sm/normal';
            entity['variant'] = tango;
            return entity;
 690:
            entity = {};
            oscar = _closure1_slot0;
            options = _closure1_slot2;
            zulu = 2;
            tango = options[zulu];
            tango = oscar.bind(golf)(tango);
            report = tango.intl;
            tango = report.string;
            zulu = options[zulu];
            zulu = oscar.bind(golf)(zulu);
            zulu = zulu.t;
            zulu = zulu.U8Rr2t;
            zulu = tango.bind(report)(zulu);
            entity['content'] = zulu;
            mike = mike.italics;
            entity['style'] = mike;
            mike = 'text-sm/normal';
            entity['variant'] = mike;
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();