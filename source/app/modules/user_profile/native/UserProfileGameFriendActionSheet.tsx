// app/modules/user_profile/native/UserProfileGameFriendActionSheet.tsx
export default (function(argFoo, argBar, argBaz, argCor, _, argFre, argPlu) {
    report = argBar;
    yankee = argBaz;
    zuuluu = argFre;
    oscard = argPlu;
    var _closure1_slot0 = report;
    var _closure1_slot1 = yankee;
    var _closure1_slot2 = oscard;
    entity = function(argFoo) { // Original name: GameFriendApplicationRow
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            zuuluu = argFoo;
            entity = zuuluu.application;
            var _closure2_slot0 = entity;
            golfie = zuuluu.userId;
            var _closure2_slot1 = golfie;
            option = zuuluu.userDisplayName;
            var _closure2_slot2 = option;
            zuuluu = _closure1_slot12;
            report = undefined;
            offset = zuuluu.bind(report)();
            oscard = _closure1_slot5;
            tangon = oscard.useCallback;
            verify = entity.id;
            zuuluu = new Array(4);
            zuuluu[0] = verify;
            verify = entity.name;
            zuuluu[1] = verify;
            zuuluu[2] = option;
            zuuluu[3] = golfie;
            michal = function() {
                zuuluu = _closure1_slot0;
                michal = _closure1_slot2;
                entity = 9;
                michal = michal[entity];
                entity = undefined;
                tangon = zuuluu.bind(entity)(michal);
                zuuluu = tangon.confirmRemoveGameFriend;
                michal = {};
                oscard = _closure2_slot2;
                michal['userDisplayName'] = oscard;
                oscard = _closure2_slot1;
                michal['userId'] = oscard;
                report = _closure2_slot0;
                oscard = report.id;
                michal['applicationId'] = oscard;
                report = report.name;
                michal['gameName'] = report;
                michal = zuuluu.bind(tangon)(michal);
                return entity;
            };
            oscard = tangon.bind(oscard)(michal, zuuluu);
            tangon = _closure1_slot10;
            option = _closure1_slot0;
            kiloes = _closure1_slot2;
            michal = 10;
            michal = kiloes[michal];
            michal = option.bind(report)(michal);
            zuuluu = michal.ActionSheetRow;
            michal = {};
            verify = _closure1_slot10;
            golfie = 11;
            golfie = kiloes[golfie];
            golfie = option.bind(report)(golfie);
            option = golfie.XSmallIcon;
            golfie = {'size': 'md', 'color': 'redesign-button-tertiary-text'};
            golfie = verify.bind(report)(option, golfie);
            michal['trailing'] = golfie;
            verify = _closure1_slot11;
            option = _closure1_slot7;
            golfie = {};
            romeon = offset.applicationNameWrapper;
            golfie['style'] = romeon;
            backup = _closure1_slot10;
            foxtra = _closure1_slot1;
            romeon = 12;
            romeon = kiloes[romeon];
            foxtra = foxtra.bind(report)(romeon);
            romeon = {};
            offset = offset.gameIcon;
            romeon['style'] = offset;
            offset = 'contain';
            romeon['resizeMode'] = offset;
            offset = {};
            sizing = entity.getIconURL;
            kiloes = 32;
            sizing = sizing.bind(entity)(kiloes);
            kiloes = null;
            output = kiloes != sizing;
            kiloes = '';
            if(!output) { _fun00002_ip = 268; continue _fun00001 }
 265:
            kiloes = sizing;
 268:
            offset['uri'] = kiloes;
            romeon['source'] = offset;
            offset = true;
            romeon['disableColor'] = offset;
            offset = entity.id;
            romeon = backup.bind(report)(foxtra, romeon, offset);
            offset = new Array(2);
            offset[0] = romeon;
            foxtra = _closure1_slot10;
            romeon = _closure1_slot0;
            backup = _closure1_slot2;
            yankee = 13;
            yankee = backup[yankee];
            yankee = romeon.bind(report)(yankee);
            romeon = yankee.Text;
            yankee = {'variant': 'text-md/semibold', 'color': 'header-primary'};
            backup = entity.name;
            yankee['children'] = backup;
            yankee = foxtra.bind(report)(romeon, yankee);
            offset[1] = yankee;
            golfie['children'] = offset;
            golfie = verify.bind(report)(option, golfie);
            michal['label'] = golfie;
            michal['onPress'] = oscard;
            entity = entity.id;
            entity = tangon.bind(report)(zuuluu, michal, entity);
            return entity;
        }
    };
    var _closure1_slot13 = entity;
    entity = global;
    option = entity.Object;
    golfie = option.defineProperty;
    tangon = {};
    entity = true;
    tangon['value'] = entity;
    entity = '__esModule';
    entity = golfie.bind(option)(zuuluu, entity, tangon);
    entity = 0;
    tangon = oscard[entity];
    entity = undefined;
    tangon = yankee.bind(entity)(tangon);
    var _closure1_slot3 = tangon;
    tangon = 1;
    tangon = oscard[tangon];
    tangon = yankee.bind(entity)(tangon);
    var _closure1_slot4 = tangon;
    tangon = 2;
    golfie = oscard[tangon];
    tangon = argCor;
    tangon = tangon.bind(entity)(golfie);
    var _closure1_slot5 = tangon;
    tangon = 3;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    golfie = tangon.ActivityIndicator;
    var _closure1_slot6 = golfie;
    tangon = tangon.View;
    var _closure1_slot7 = tangon;
    tangon = 4;
    tangon = oscard[tangon];
    tangon = yankee.bind(entity)(tangon);
    var _closure1_slot8 = tangon;
    tangon = 5;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    tangon = tangon.RelationshipTypes;
    var _closure1_slot9 = tangon;
    tangon = 6;
    tangon = oscard[tangon];
    tangon = report.bind(entity)(tangon);
    golfie = tangon.jsx;
    var _closure1_slot10 = golfie;
    tangon = tangon.jsxs;
    var _closure1_slot11 = tangon;
    tangon = 7;
    tangon = oscard[tangon];
    option = report.bind(entity)(tangon);
    golfie = option.createStyles;
    tangon = {};
    verify = {'flexDirection': 'row', 'justifyContent': 'flex-start', 'alignItems': 'center', 'gap': 12};
    tangon['applicationNameWrapper'] = verify;
    verify = {'width': 32, 'height': 32};
    offset = 8;
    offset = oscard[offset];
    offset = yankee.bind(entity)(offset);
    offset = offset.radii;
    offset = offset.sm;
    verify['borderRadius'] = offset;
    tangon['gameIcon'] = verify;
    tangon = golfie.bind(option)(tangon);
    var _closure1_slot12 = tangon;
    tangon = 23;
    tangon = oscard[tangon];
    oscard = report.bind(entity)(tangon);
    report = oscard.fileFinishedImporting;
    tangon = 'modules/user_profile/native/UserProfileGameFriendActionSheet.tsx';
    tangon = report.bind(oscard)(tangon);
    michal = function(argFoo) { // Original name: UserProfileGameFriendActionSheet
        _fun00003: for(var _fun00004_ip = 0; ; ) switch(_fun00004_ip) {
 0:
            entity = argFoo;
            michal = entity.user;
            var _closure2_slot0 = michal;
            golfie = entity.guildId;
            oscard = entity.channelId;
            echoed = _closure1_slot0;
            yankee = _closure1_slot2;
            entity = 14;
            entity = yankee[entity];
            tangon = undefined;
            report = echoed.bind(tangon)(entity);
            zuuluu = report.useGameFriendsForUser;
            entity = michal.id;
            entity = zuuluu.bind(report)(entity);
            var _closure2_slot1 = entity;
            option = _closure1_slot5;
            report = option.useMemo;
            zuuluu = new Array(1);
            zuuluu[0] = entity;
            entity = function() {
                zuuluu = _closure2_slot1;
                michal = zuuluu.map;
                entity = function(argFoo) {
                    entity = argFoo;
                    entity = entity.applicationId;
                    return entity;
                };
                entity = michal.bind(zuuluu)(entity);
                return entity;
            };
            entity = report.bind(option)(entity, zuuluu);
            var _closure2_slot2 = entity;
            report = _closure1_slot1;
            zuuluu = 15;
            zuuluu = yankee[zuuluu];
            zuuluu = report.bind(tangon)(zuuluu);
            romeon = zuuluu.bind(tangon)(entity);
            zuuluu = 16;
            zuuluu = yankee[zuuluu];
            report = report.bind(tangon)(zuuluu);
            zuuluu = report.useName;
            zuuluu = zuuluu.bind(report)(golfie, oscard, michal);
            var _closure2_slot3 = zuuluu;
            oscard = _closure1_slot5;
            report = oscard.useState;
            zuuluu = function() {
                _fun00005: for(var _fun00006_ip = 0; ; ) switch(_fun00006_ip) {
 0:
                    report = _closure1_slot8;
                    tangon = report.isFriend;
                    zuuluu = _closure2_slot0;
                    zuuluu = zuuluu.id;
                    zuuluu = tangon.bind(report)(zuuluu);
                    if(zuuluu) { _fun00006_ip = 117; continue _fun00005 }
 33:
                    report = _closure1_slot8;
                    tangon = report.isBlockedOrIgnored;
                    zuuluu = _closure2_slot0;
                    zuuluu = zuuluu.id;
                    zuuluu = tangon.bind(report)(zuuluu);
                    if(zuuluu) { _fun00006_ip = 117; continue _fun00005 }
 60:
                    tangon = _closure1_slot8;
                    zuuluu = tangon.getRelationshipType;
                    entity = _closure2_slot0;
                    entity = entity.id;
                    zuuluu = zuuluu.bind(tangon)(entity);
                    entity = _closure1_slot9;
                    entity = entity.PENDING_OUTGOING;
                    entity = zuuluu !== entity;
                    if(!entity) { _fun00006_ip = 115; continue _fun00005 }
 101:
                    michal = _closure1_slot9;
                    michal = michal.PENDING_INCOMING;
                    entity = zuuluu !== michal;
 115:
                    return entity;
 117:
                    entity = false;
                    return entity;
                }
            };
            zuuluu = report.bind(oscard)(zuuluu);
            offset = _closure1_slot4;
            report = 1;
            zuuluu = offset.bind(tangon)(zuuluu, report);
            golfie = 0;
            oscard = zuuluu[golfie];
            option = _closure1_slot5;
            zuuluu = option.useState;
            sizing = false;
            zuuluu = zuuluu.bind(option)(sizing);
            option = 2;
            zuuluu = offset.bind(tangon)(zuuluu, option);
            foxtra = zuuluu[golfie];
            zuuluu = zuuluu[report];
            var _closure2_slot4 = zuuluu;
            kiloes = _closure1_slot5;
            zuuluu = kiloes.useState;
            zuuluu = zuuluu.bind(kiloes)(sizing);
            zuuluu = offset.bind(tangon)(zuuluu, option);
            kiloes = zuuluu[golfie];
            zuuluu = zuuluu[report];
            var _closure2_slot5 = zuuluu;
            golfie = _closure1_slot5;
            report = golfie.useCallback;
            option = _closure1_slot3;
            zuuluu = function* () {
                entity = function* () { // Original name: ?anon_0_
                    _fun00007: for(var _fun00008_ip = 0; ; ) switch(_fun00008_ip) {
 0:
                        StartGenerator();
                        ResumeGenerator(result_out_reg=0, return_bool_out_reg=1);
                        if(michal) { _fun00008_ip = 198; continue _fun00007 }
 10:
                        michal = _closure2_slot4;
                        report = undefined;
                        tangon = true;
                        michal = michal.bind(report)(tangon);
                        michal = _closure2_slot5;
                        michal = michal.bind(report)(tangon);
 35: // try_start_0 // try_start_1
                        oscard = _closure1_slot1;
                        golfie = _closure1_slot2;
                        michal = 17;
                        michal = golfie[michal];
                        golfie = oscard.bind(report)(michal);
                        oscard = golfie.addRelationship;
                        michal = {};
                        option = _closure2_slot0;
                        option = option.id;
                        michal['userId'] = option;
                        option = {};
                        verify = 'User Profile Action Sheet';
                        option['location'] = verify;
                        michal['context'] = option;
                        michal = oscard.bind(golfie)(michal);
                        SaveGenerator(address=104);
 102:
                        return michal;
 104:
                        ResumeGenerator(result_out_reg=1, return_bool_out_reg=5);
                        if(oscard) { _fun00008_ip = 142; continue _fun00007 }
 110:
                        oscard = _closure1_slot0;
                        golfie = _closure1_slot2;
                        tangon = 18;
                        tangon = golfie[tangon];
                        oscard = oscard.bind(report)(tangon);
                        tangon = oscard.presentAddedFriendToast;
                        tangon = tangon.bind(oscard)();
 140: // try_end0
                        _fun00008_ip = 169; continue _fun00007;
 142: // try_end1
                        oscard = _closure2_slot5;
                        tangon = false;
                        tangon = oscard.bind(report)(tangon);
                        return michal;
 156: // try_start_2 // catch_target0
                        CatchBlockStart(arg_register=1);
                        tangon = _closure2_slot4;
                        michal = false;
                        michal = tangon.bind(report)(michal);
 169: // try_end2
                        tangon = _closure2_slot5;
                        michal = false;
                        michal = tangon.bind(report)(michal);
                        return report;
 183: // catch_target1 // catch_target2
                        CatchBlockStart(arg_register=1);
                        tangon = _closure2_slot5;
                        zuuluu = false;
                        zuuluu = tangon.bind(report)(zuuluu);
                        throw michal;
 198:
                        return entity;
                    }
                };
                return entity;
            };
            zuuluu = option.bind(tangon)(zuuluu);
            option = michal.id;
            michal = new Array(1);
            michal[0] = option;
            sizing = report.bind(golfie)(zuuluu, michal);
            report = _closure1_slot5;
            zuuluu = report.useEffect;
            michal = new Array(1);
            michal[0] = entity;
            entity = function() {
                _fun00009: for(var _fun00010_ip = 0; ; ) switch(_fun00010_ip) {
 0:
                    entity = _closure2_slot2;
                    michal = entity.length;
                    entity = 0;
                    if(!(entity === michal)) { _fun00010_ip = 53; continue _fun00009 }
 18:
                    zuuluu = _closure1_slot1;
                    michal = _closure1_slot2;
                    entity = 19;
                    michal = michal[entity];
                    entity = undefined;
                    michal = zuuluu.bind(entity)(michal);
                    entity = michal.hideActionSheet;
                    entity = entity.bind(michal)();
 53:
                    entity = undefined;
                    return entity;
                }
            };
            entity = zuuluu.bind(report)(entity, michal);
            zuuluu = _closure1_slot11;
            entity = 20;
            entity = yankee[entity];
            entity = echoed.bind(tangon)(entity);
            michal = entity.ActionSheet;
            entity = {};
            option = _closure1_slot10;
            report = 21;
            report = yankee[report];
            report = echoed.bind(tangon)(report);
            golfie = report.BottomSheetTitleHeader;
            report = {};
            source = 22;
            offset = yankee[source];
            offset = echoed.bind(tangon)(offset);
            result = offset.intl;
            output = result.string;
            offset = yankee[source];
            offset = echoed.bind(tangon)(offset);
            offset = offset.t;
            offset = offset.Uv/eT0;
            offset = output.bind(result)(offset);
            report['title'] = offset;
            report = option.bind(tangon)(golfie, report);
            entity['header'] = report;
            option = _closure1_slot10;
            offset = 10;
            report = yankee[offset];
            report = echoed.bind(tangon)(report);
            report = report.ActionSheetRow;
            golfie = report.Group;
            report = {};
            output = yankee[source];
            output = echoed.bind(tangon)(output);
            result = output.intl;
            output = result.string;
            yankee = yankee[source];
            yankee = echoed.bind(tangon)(yankee);
            yankee = yankee.t;
            yankee = yankee.YpCiMj;
            yankee = output.bind(result)(yankee);
            report['title'] = yankee;
            yankee = romeon.map;
            verify = function(argFoo) {
                _fun00011: for(var _fun00012_ip = 0; ; ) switch(_fun00012_ip) {
 0:
                    michal = argFoo;
                    entity = null;
                    entity = entity != michal;
                    if(!entity) { _fun00012_ip = 68; continue _fun00011 }
 12:
                    oscard = _closure1_slot10;
                    report = _closure1_slot13;
                    tangon = {};
                    tangon['application'] = michal;
                    golfie = _closure2_slot3;
                    tangon['userDisplayName'] = golfie;
                    zuuluu = _closure2_slot0;
                    zuuluu = zuuluu.id;
                    tangon['userId'] = zuuluu;
                    zuuluu = michal.id;
                    michal = undefined;
                    entity = oscard.bind(michal)(report, tangon, zuuluu);
 68:
                    return entity;
                }
            };
            verify = yankee.bind(romeon)(verify);
            report['children'] = verify;
            golfie = option.bind(tangon)(golfie, report);
            report = new Array(2);
            report[0] = golfie;
            if(!oscard) { _fun00004_ip = 811; continue _fun00003 }
 554:
            verify = _closure1_slot10;
            update = _closure1_slot0;
            output = _closure1_slot2;
            golfie = output[offset];
            golfie = update.bind(tangon)(golfie);
            golfie = golfie.ActionSheetRow;
            option = golfie.Group;
            golfie = {};
            yankee = output[source];
            yankee = update.bind(tangon)(yankee);
            result = yankee.intl;
            romeon = result.string;
            yankee = output[source];
            yankee = update.bind(tangon)(yankee);
            yankee = yankee.t;
            yankee = yankee.GbsGCg;
            yankee = romeon.bind(result)(yankee);
            golfie['title'] = yankee;
            romeon = _closure1_slot10;
            offset = output[offset];
            offset = update.bind(tangon)(offset);
            yankee = offset.ActionSheetRow;
            offset = {};
            result = output[source];
            result = update.bind(tangon)(result);
            ctrled = result.intl;
            echoed = ctrled.string;
            result = output[source];
            result = update.bind(tangon)(result);
            result = result.t;
            result = result.LAcY7u;
            result = echoed.bind(ctrled)(result);
            offset['label'] = result;
            result = output[source];
            result = update.bind(tangon)(result);
            echoed = result.intl;
            result = echoed.string;
            output = output[source];
            output = update.bind(tangon)(output);
            output = output.t;
            output = output.YTvOU1;
            output = result.bind(echoed)(output);
            offset['subLabel'] = output;
            offset['onPress'] = sizing;
            offset['disabled'] = foxtra;
            foxtra = null;
            if(!kiloes) { _fun00004_ip = 790; continue _fun00003 }
 774:
            sizing = _closure1_slot10;
            kiloes = _closure1_slot6;
            backup = {};
            foxtra = sizing.bind(tangon)(kiloes, backup);
 790:
            offset['trailing'] = foxtra;
            offset = romeon.bind(tangon)(yankee, offset);
            golfie['children'] = offset;
            oscard = verify.bind(tangon)(option, golfie);
 811:
            report[1] = oscard;
            entity['children'] = report;
            entity = zuuluu.bind(tangon)(michal, entity);
            return entity;
        }
    };
    zuuluu['default'] = michal;
    return entity;
})();