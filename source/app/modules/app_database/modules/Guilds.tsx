// app/modules/app_database/modules/Guilds.tsx
export default (function(argFoo, argBar, argBaz, argCor, _, argFre, argPlu) {
    report = argBar;
    golfie = argBaz;
    zuuluu = argFre;
    oscard = argPlu;
    var _closure1_slot0 = report;
    var _closure1_slot1 = golfie;
    entity = argCor;
    var _closure1_slot2 = entity;
    var _closure1_slot3 = oscard;
    entity = function(argFoo, argBar) { // Original name: _createForOfIteratorHelperLoose
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            zuuluu = argFoo;
            var _closure2_slot0 = zuuluu;
            report = global;
            michal = report.Symbol;
            tangon = 'undefined';
            michal = typeof michal;
            michal = tangon !== michal;
            if(!michal) { _fun00002_ip = 46; continue _fun00001 }
 30:
            tangon = report.Symbol;
            tangon = tangon.iterator;
            michal = zuuluu[tangon];
 46:
            if(michal) { _fun00002_ip = 55; continue _fun00001 }
 49:
            michal = zuuluu.@@iterator;
 55:
            if(michal) { _fun00002_ip = 343; continue _fun00001 }
 61:
            oscard = report.Array;
            tangon = oscard.isArray;
            oscard = tangon.bind(oscard)(zuuluu);
            tangon = michal;
            if(oscard) { _fun00002_ip = 323; continue _fun00001 }
 86:
            option = undefined;
            oscard = undefined;
            if(!zuuluu) { _fun00002_ip = 283; continue _fun00001 }
 96:
            verify = 'string';
            golfie = typeof zuuluu;
            if(!(verify !== golfie)) { _fun00002_ip = 270; continue _fun00001 }
 110:
            golfie = {};
            verify = golfie.toString;
            golfie = verify.call;
            yankee = golfie.bind(verify)(zuuluu);
            offset = yankee.slice;
            verify = 8;
            golfie = -1;
            verify = offset.bind(yankee)(verify, golfie);
            golfie = 'Object';
            golfie = golfie === verify;
            if(!golfie) { _fun00002_ip = 163; continue _fun00001 }
 158:
            golfie = zuuluu.constructor;
 163:
            offset = verify;
            if(!golfie) { _fun00002_ip = 179; continue _fun00001 }
 169:
            golfie = zuuluu.constructor;
            offset = golfie.name;
 179:
            golfie = 'Map';
            if(!(golfie !== offset)) { _fun00002_ip = 249; continue _fun00001 }
 187:
            golfie = 'Set';
            if(!(golfie !== offset)) { _fun00002_ip = 249; continue _fun00001 }
 195:
            golfie = 'Arguments';
            if(!(golfie !== offset)) { _fun00002_ip = 234; continue _fun00001 }
 205:
            verify = /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/;
            golfie = verify.test;
            verify = golfie.bind(verify)(offset);
            golfie = undefined;
            if(!verify) { _fun00002_ip = 247; continue _fun00001 }
 234:
            verify = _closure1_slot12;
            golfie = verify.bind(option)(zuuluu, option);
 247:
            _fun00002_ip = 265; continue _fun00001;
 249:
            offset = report.Array;
            verify = offset.from;
            golfie = verify.bind(offset)(zuuluu);
 265:
            oscard = golfie;
            _fun00002_ip = 283; continue _fun00001;
 270:
            golfie = _closure1_slot12;
            oscard = golfie.bind(option)(zuuluu, option);
 283:
            tangon = oscard;
            if(tangon) { _fun00002_ip = 323; continue _fun00001 }
 289:
            golfie = report.TypeError;
            report = golfie.prototype;
            oscard = Object.create(report, {constructor: {value: golfie}});
            foxtra = 'Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.';
            backup = oscard;
            report = new backup[golfie](foxtra, romeon);
            report = report instanceof Object ? report : oscard;
            throw report;
 323:
            if(!tangon) { _fun00002_ip = 330; continue _fun00001 }
 326:
            _closure2_slot0 = tangon;
 330:
            tangon = 0;
            var _closure2_slot1 = tangon;
            entity = function() {
                _fun00003: for(var _fun00004_ip = 0; ; ) switch(_fun00004_ip) {
 0:
                    michal = _closure2_slot1;
                    entity = _closure2_slot0;
                    entity = entity.length;
                    if(!(!(michal >= entity))) { _fun00004_ip = 56; continue _fun00003 }
 20:
                    entity = {};
                    michal = false;
                    entity['done'] = michal;
                    zuuluu = _closure2_slot0;
                    michal = _closure2_slot1;
                    michal = parseFloat(michal);
                    tangon = michal + 1;
                    _closure2_slot1 = tangon;
                    michal = zuuluu[michal];
                    entity['value'] = michal;
                    _fun00004_ip = 67; continue _fun00003;
 56:
                    michal = {};
                    zuuluu = true;
                    michal['done'] = zuuluu;
                    entity = michal;
 67:
                    return entity;
                }
            };
            return entity;
 343:
            entity = michal.call;
            zuuluu = entity.bind(michal)(zuuluu);
            michal = zuuluu.next;
            entity = michal.bind;
            entity = entity.bind(michal)(zuuluu);
            return entity;
        }
    };
    var _closure1_slot11 = entity;
    entity = function(argFoo, argBar) { // Original name: _arrayLikeToArray
        _fun00005: for(var _fun00006_ip = 0; ; ) switch(_fun00006_ip) {
 0:
            tangon = argFoo;
            entity = null;
            zuuluu = undefined;
            entity = entity == zuuluu;
            if(entity) { _fun00006_ip = 23; continue _fun00005 }
 14:
            michal = tangon.length;
            entity = zuuluu > michal;
 23:
            michal = undefined;
            if(!entity) { _fun00006_ip = 33; continue _fun00005 }
 28:
            michal = tangon.length;
 33:
            entity = global;
            entity = entity.Array;
            entity = entity.bind(zuuluu)(michal);
            zuuluu = 0;
            report = zuuluu < michal;
            if(!report) { _fun00006_ip = 70; continue _fun00005 }
 55:
            report = tangon[zuuluu];
            entity[zuuluu] = report;
            zuuluu = zuuluu + 1;
            if(zuuluu < michal) { _fun00006_ip = 55; continue _fun00005 }
 70:
            return entity;
        }
    };
    var _closure1_slot12 = entity;
    entity = global;
    verify = entity.Object;
    option = verify.defineProperty;
    tangon = {};
    entity = true;
    tangon['value'] = entity;
    entity = '__esModule';
    entity = option.bind(verify)(zuuluu, entity, tangon);
    entity = 0;
    tangon = oscard[entity];
    entity = undefined;
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot4 = tangon;
    tangon = 1;
    tangon = oscard[tangon];
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot5 = tangon;
    tangon = 2;
    tangon = oscard[tangon];
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot6 = tangon;
    tangon = 3;
    tangon = oscard[tangon];
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot7 = tangon;
    tangon = 4;
    tangon = oscard[tangon];
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot8 = tangon;
    tangon = 5;
    tangon = oscard[tangon];
    tangon = golfie.bind(entity)(tangon);
    var _closure1_slot9 = tangon;
    tangon = 6;
    tangon = oscard[tangon];
    option = golfie.bind(entity)(tangon);
    tangon = option.prototype;
    golfie = Object.create(tangon, {constructor: {value: option}});
    romeon = 'Guilds';
    foxtra = golfie;
    tangon = new foxtra[option](romeon, yankee);
    tangon = tangon instanceof Object ? tangon : golfie;
    var _closure1_slot10 = tangon;
    michal = function() {
        tangon = _closure1_slot6;
        zuuluu = function() { // Original name: Guilds
            zuuluu = this;
            var _closure3_slot0 = zuuluu;
            report = _closure1_slot5;
            michal = _closure2_slot2;
            entity = undefined;
            michal = report.bind(entity)(zuuluu, michal);
            michal = {};
            report = function(argFoo, argBar) { // Original name: BACKGROUND_SYNC
                tangon = _closure3_slot0;
                zuuluu = tangon.handleBackgroundSync;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['BACKGROUND_SYNC'] = report;
            report = function(argFoo, argBar) { // Original name: CONNECTION_OPEN
                tangon = _closure3_slot0;
                zuuluu = tangon.handleConnectionOpen;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['CONNECTION_OPEN'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_CREATE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildCreate;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_CREATE'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_DELETE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildDelete;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_DELETE'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_MEMBER_ADD
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildMemberAdd;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_MEMBER_ADD'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_MEMBER_UPDATE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildMemberUpdate;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_MEMBER_UPDATE'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_ROLE_CREATE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildRoleChange;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_ROLE_CREATE'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_ROLE_DELETE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildRoleDelete;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_ROLE_DELETE'] = report;
            report = function(argFoo, argBar) { // Original name: GUILD_ROLE_UPDATE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildRoleChange;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_ROLE_UPDATE'] = report;
            tangon = function(argFoo, argBar) { // Original name: GUILD_UPDATE
                tangon = _closure3_slot0;
                zuuluu = tangon.handleGuildUpdate;
                michal = argFoo;
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
                return entity;
            };
            michal['GUILD_UPDATE'] = tangon;
            zuuluu['actions'] = michal;
            return entity;
        };
        var _closure2_slot2 = zuuluu;
        report = {};
        michal = 'getAsync';
        report['key'] = michal;
        option = _closure1_slot4;
        michal = undefined;
        entity = function* (argFoo) {
            entity = function* (argFoo) { // Original name: ?anon_0_
                _fun00007: for(var _fun00008_ip = 0; ; ) switch(_fun00008_ip) {
 0:
                    StartGenerator();
                    ResumeGenerator(result_out_reg=0, return_bool_out_reg=1);
                    if(michal) { _fun00008_ip = 162; continue _fun00007 }
 10:
                    zuuluu = global;
                    tangon = zuuluu.performance;
                    michal = tangon.now;
                    golfie = michal.bind(tangon)();
                    oscard = _closure1_slot1;
                    report = _closure1_slot3;
                    michal = 7;
                    report = report[michal];
                    michal = undefined;
                    oscard = oscard.bind(michal)(report);
                    report = oscard.guilds;
                    michal = argFoo;
                    report = report.bind(oscard)(michal);
                    michal = report.getMany;
                    michal = michal.bind(report)();
                    SaveGenerator(address=80);
 78:
                    return michal;
 80:
                    ResumeGenerator(result_out_reg=1, return_bool_out_reg=4);
                    if(report) { _fun00008_ip = 159; continue _fun00007 }
 86:
                    oscard = zuuluu.performance;
                    report = oscard.now;
                    oscard = report.bind(oscard)();
                    report = _closure1_slot10;
                    tangon = report.verbose;
                    backup = oscard - golfie;
                    romeon = michal.length;
                    zuuluu = zuuluu.HermesInternal;
                    option = zuuluu.concat;
                    kiloes = 'loaded in ';
                    foxtra = 'ms (guilds: ';
                    yankee = ')';
                    zuuluu = kiloes[option](backup, foxtra, romeon, yankee, offset);
                    zuuluu = tangon.bind(report)(zuuluu);
                    return michal;
 159:
                    return michal;
 162:
                    return entity;
                }
            };
            return entity;
        };
        entity = option.bind(michal)(entity);
        var _closure2_slot1 = entity;
        entity = function() { // Original name: getAsync
            entity = undefined;
            tangon = _closure2_slot1;
            zuuluu = tangon.apply;
            entity = arguments;
            michal = entity;
            entity = this;
            entity = zuuluu.bind(tangon)(entity, michal);
            return entity;
        };
        report['value'] = entity;
        entity = new Array(16);
        entity[0] = report;
        report = {};
        golfie = 'getOneAsync';
        report['key'] = golfie;
        golfie = function* (argFoo, argBar) {
            entity = function* (argFoo, argBar) { // Original name: ?anon_0_
                _fun00009: for(var _fun00010_ip = 0; ; ) switch(_fun00010_ip) {
 0:
                    StartGenerator();
                    ResumeGenerator(result_out_reg=0, return_bool_out_reg=1);
                    if(michal) { _fun00010_ip = 75; continue _fun00009 }
 7:
                    tangon = _closure1_slot1;
                    zuuluu = _closure1_slot3;
                    michal = 7;
                    zuuluu = zuuluu[michal];
                    michal = undefined;
                    tangon = tangon.bind(michal)(zuuluu);
                    zuuluu = tangon.guilds;
                    michal = argFoo;
                    tangon = zuuluu.bind(tangon)(michal);
                    zuuluu = tangon.get;
                    michal = argBar;
                    michal = zuuluu.bind(tangon)(michal);
                    SaveGenerator(address=63);
 61:
                    return michal;
 63:
                    ResumeGenerator(result_out_reg=1, return_bool_out_reg=2);
                    if(zuuluu) { _fun00010_ip = 72; continue _fun00009 }
 69:
                    return michal;
 72:
                    return michal;
 75:
                    return entity;
                }
            };
            return entity;
        };
        golfie = option.bind(michal)(golfie);
        var _closure2_slot0 = golfie;
        golfie = function() { // Original name: getOneAsync
            entity = undefined;
            tangon = _closure2_slot0;
            zuuluu = tangon.apply;
            entity = arguments;
            michal = entity;
            entity = this;
            entity = zuuluu.bind(tangon)(entity, michal);
            return entity;
        };
        report['value'] = golfie;
        entity[1] = report;
        report = {};
        golfie = 'handleBackgroundSync';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00011: for(var _fun00012_ip = 0; ; ) switch(_fun00012_ip) {
 0:
                foxtra = argBar;
                romeon = this;
                zuuluu = _closure1_slot11;
                entity = argFoo;
                michal = entity.guilds;
                entity = undefined;
                offset = zuuluu.bind(entity)(michal);
                zuuluu = offset.bind(entity)();
                michal = zuuluu.done;
                verify = 8;
                option = 9;
                golfie = 'partial';
                oscard = null;
                report = 'unavailable';
                tangon = zuuluu;
                zuuluu = undefined;
                if(michal) { _fun00012_ip = 362; continue _fun00011 }
 65:
                echoed = tangon.value;
                michal = echoed.data_mode;
                if(!(report !== michal)) { _fun00012_ip = 360; continue _fun00011 }
 83:
                kiloes = _closure1_slot9;
                backup = kiloes.getGuild;
                michal = echoed.id;
                source = backup.bind(kiloes)(michal);
                if(!(oscard != source)) { _fun00012_ip = 340; continue _fun00011 }
 109:
                kiloes = _closure1_slot9;
                backup = kiloes.getRoles;
                michal = echoed.id;
                michal = backup.bind(kiloes)(michal);
                backup = echoed.data_mode;
                if(!(golfie !== backup)) { _fun00012_ip = 181; continue _fun00011 }
 139:
                kiloes = _closure1_slot0;
                backup = _closure1_slot3;
                backup = backup[option];
                output = kiloes.bind(entity)(backup);
                sizing = output.sortServerRoles;
                kiloes = echoed.id;
                backup = echoed.roles;
                update = sizing.bind(output)(kiloes, backup);
                _fun00012_ip = 243; continue _fun00011;
 181:
                kiloes = _closure1_slot2;
                backup = _closure1_slot3;
                backup = backup[verify];
                result = kiloes.bind(entity)(backup);
                output = result.filterRoleDeletes;
                config = echoed.id;
                backup = echoed.partial_updates;
                vacuum = backup.roles;
                backup = echoed.partial_updates;
                ctrled = backup.deleted_role_ids;
                record = result;
                sequen = michal;
                update = record[output](config, sequen, vacuum, ctrled, source);
 243:
                kiloes = romeon.put;
                sizing = _closure1_slot2;
                backup = _closure1_slot3;
                output = backup[verify];
                result = sizing.bind(entity)(output);
                output = result.attachSerializedData;
                backup = backup[verify];
                sizing = sizing.bind(entity)(backup);
                backup = sizing.fromBackgroundSync;
                sizing = backup.bind(sizing)(echoed, source);
                backup = {};
                backup['roles'] = update;
                source = _closure1_slot8;
                update = source.getSelfMember;
                echoed = echoed.id;
                echoed = update.bind(source)(echoed);
                backup['member'] = echoed;
                backup = output.bind(result)(sizing, backup);
                backup = kiloes.bind(romeon)(backup, foxtra);
                zuuluu = michal;
 340:
                backup = offset.bind(entity)();
                michal = backup.done;
                tangon = backup;
                if(michal) { _fun00012_ip = 362; continue _fun00011 }
 355:
                _fun00012_ip = 65; continue _fun00011;
 360:
                return entity;
 362:
                return entity;
            }
        };
        report['value'] = golfie;
        entity[2] = report;
        report = {};
        golfie = 'handleConnectionOpen';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00013: for(var _fun00014_ip = 0; ; ) switch(_fun00014_ip) {
 0:
                oscard = argBar;
                report = this;
                entity = report.clear;
                entity = entity.bind(report)(oscard);
                zuuluu = _closure1_slot11;
                entity = argFoo;
                michal = entity.guilds;
                entity = undefined;
                tangon = zuuluu.bind(entity)(michal);
                zuuluu = tangon.bind(entity)();
                michal = zuuluu.done;
                if(michal) { _fun00014_ip = 84; continue _fun00013 }
 52:
                golfie = zuuluu.value;
                michal = report.putOne;
                michal = michal.bind(report)(golfie, oscard);
                golfie = tangon.bind(entity)();
                michal = golfie.done;
                zuuluu = golfie;
                if(!michal) { _fun00014_ip = 52; continue _fun00013 }
 84:
                return entity;
            }
        };
        report['value'] = golfie;
        entity[3] = report;
        report = {};
        golfie = 'handleGuildCreate';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            tangon = this;
            zuuluu = tangon.putOne;
            entity = argFoo;
            michal = entity.guild;
            entity = argBar;
            entity = zuuluu.bind(tangon)(michal, entity);
            entity = undefined;
            return entity;
        };
        report['value'] = golfie;
        entity[4] = report;
        report = {};
        golfie = 'handleGuildUpdate';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            option = argFoo;
            report = this;
            zuuluu = _closure1_slot9;
            michal = zuuluu.getGuild;
            entity = option.guild;
            entity = entity.id;
            offset = michal.bind(zuuluu)(entity);
            zuuluu = _closure1_slot2;
            romeon = _closure1_slot3;
            michal = 8;
            tangon = romeon[michal];
            entity = undefined;
            golfie = zuuluu.bind(entity)(tangon);
            oscard = golfie.fromServerUpdate;
            tangon = option.guild;
            golfie = oscard.bind(golfie)(tangon, offset);
            tangon = report.put;
            michal = romeon[michal];
            oscard = zuuluu.bind(entity)(michal);
            zuuluu = oscard.attachSerializedData;
            michal = {};
            yankee = _closure1_slot0;
            offset = 9;
            offset = romeon[offset];
            foxtra = yankee.bind(entity)(offset);
            romeon = foxtra.sortServerRoles;
            offset = option.guild;
            yankee = offset.id;
            offset = option.guild;
            offset = offset.roles;
            offset = romeon.bind(foxtra)(yankee, offset);
            michal['roles'] = offset;
            offset = _closure1_slot8;
            verify = offset.getSelfMember;
            option = option.guild;
            option = option.id;
            option = verify.bind(offset)(option);
            michal['member'] = option;
            zuuluu = zuuluu.bind(oscard)(golfie, michal);
            michal = argBar;
            michal = tangon.bind(report)(zuuluu, michal);
            return entity;
        };
        report['value'] = golfie;
        entity[5] = report;
        report = {};
        golfie = 'handleGuildDelete';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            tangon = this;
            zuuluu = tangon.delete;
            entity = argFoo;
            entity = entity.guild;
            michal = entity.id;
            entity = argBar;
            entity = zuuluu.bind(tangon)(michal, entity);
            entity = undefined;
            return entity;
        };
        report['value'] = golfie;
        entity[6] = report;
        report = {};
        golfie = 'handleGuildRoleChange';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00015: for(var _fun00016_ip = 0; ; ) switch(_fun00016_ip) {
 0:
                golfie = argFoo;
                tangon = this;
                zuuluu = _closure1_slot9;
                michal = zuuluu.getGuild;
                entity = golfie.guildId;
                oscard = michal.bind(zuuluu)(entity);
                michal = zuuluu.getRoles;
                entity = golfie.guildId;
                offset = michal.bind(zuuluu)(entity);
                entity = null;
                if(!(entity != oscard)) { _fun00016_ip = 165; continue _fun00015 }
 50:
                zuuluu = tangon.put;
                report = _closure1_slot2;
                michal = _closure1_slot3;
                entity = 8;
                michal = michal[entity];
                entity = undefined;
                report = report.bind(entity)(michal);
                michal = report.attachSerializedData;
                entity = {};
                verify = {};
                foxtra = verify;
                romeon = offset;
                offset = copyDataProperties(foxtra, romeon);
                offset = golfie.role;
                yankee = offset.id;
                offset = golfie.role;
                verify[yankee] = offset;
                entity['roles'] = verify;
                verify = _closure1_slot8;
                option = verify.getSelfMember;
                golfie = golfie.guildId;
                golfie = option.bind(verify)(golfie);
                entity['member'] = golfie;
                michal = michal.bind(report)(oscard, entity);
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
 165:
                entity = undefined;
                return entity;
            }
        };
        report['value'] = golfie;
        entity[7] = report;
        report = {};
        golfie = 'handleGuildRoleDelete';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00017: for(var _fun00018_ip = 0; ; ) switch(_fun00018_ip) {
 0:
                report = argFoo;
                tangon = this;
                var _closure3_slot0 = report;
                golfie = _closure1_slot9;
                oscard = golfie.getGuild;
                michal = report.guildId;
                oscard = oscard.bind(golfie)(michal);
                michal = null;
                if(!(michal != oscard)) { _fun00018_ip = 215; continue _fun00017 }
 43:
                option = {};
                offset = _closure1_slot9;
                verify = offset.getRoles;
                golfie = report.guildId;
                yankee = verify.bind(offset)(golfie);
                romeon = option;
                golfie = copyDataProperties(romeon, yankee);
                golfie = report.roleId;
                golfie = delete option[golfie];
                verify = _closure1_slot8;
                golfie = verify.getSelfMember;
                report = report.guildId;
                report = golfie.bind(verify)(report);
                golfie = report;
                if(!(michal != golfie)) { _fun00018_ip = 154; continue _fun00017 }
 109:
                michal = {};
                romeon = michal;
                yankee = report;
                verify = copyDataProperties(romeon, yankee);
                verify = report.roles;
                report = verify.filter;
                zuuluu = function(argFoo) {
                    entity = _closure3_slot0;
                    michal = entity.roleId;
                    entity = argFoo;
                    entity = entity !== michal;
                    return entity;
                };
                report = report.bind(verify)(zuuluu);
                zuuluu = 'roles';
                michal[zuuluu] = report;
                golfie = michal;
 154:
                zuuluu = tangon.put;
                report = _closure1_slot2;
                michal = _closure1_slot3;
                entity = 8;
                michal = michal[entity];
                entity = undefined;
                report = report.bind(entity)(michal);
                michal = report.attachSerializedData;
                entity = {};
                entity['roles'] = option;
                entity['member'] = golfie;
                michal = michal.bind(report)(oscard, entity);
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
 215:
                entity = undefined;
                return entity;
            }
        };
        report['value'] = golfie;
        entity[8] = report;
        report = {};
        golfie = 'handleGuildMemberAdd';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00019: for(var _fun00020_ip = 0; ; ) switch(_fun00020_ip) {
 0:
                entity = argFoo;
                tangon = this;
                zuuluu = entity.joinedAt;
                michal = null;
                if(!(michal != zuuluu)) { _fun00020_ip = 196; continue _fun00019 }
 21:
                zuuluu = entity.user;
                report = zuuluu.id;
                oscard = _closure1_slot7;
                zuuluu = oscard.getId;
                zuuluu = zuuluu.bind(oscard)();
                if(!(report === zuuluu)) { _fun00020_ip = 196; continue _fun00019 }
 55:
                oscard = _closure1_slot9;
                report = oscard.getGuild;
                zuuluu = entity.guildId;
                golfie = report.bind(oscard)(zuuluu);
                if(!(michal != golfie)) { _fun00020_ip = 196; continue _fun00019 }
 78:
                zuuluu = tangon.put;
                oscard = _closure1_slot2;
                report = _closure1_slot3;
                michal = 8;
                report = report[michal];
                michal = undefined;
                oscard = oscard.bind(michal)(report);
                report = oscard.attachSerializedData;
                michal = golfie.updateJoinedAt;
                entity = entity.joinedAt;
                michal = michal.bind(golfie)(entity);
                entity = {};
                yankee = _closure1_slot9;
                offset = yankee.getRoles;
                verify = golfie.id;
                verify = offset.bind(yankee)(verify);
                entity['roles'] = verify;
                verify = _closure1_slot8;
                option = verify.getSelfMember;
                golfie = golfie.id;
                golfie = option.bind(verify)(golfie);
                entity['member'] = golfie;
                michal = report.bind(oscard)(michal, entity);
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
 196:
                entity = undefined;
                return entity;
            }
        };
        report['value'] = golfie;
        entity[9] = report;
        report = {};
        golfie = 'handleGuildMemberUpdate';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00021: for(var _fun00022_ip = 0; ; ) switch(_fun00022_ip) {
 0:
                verify = argFoo;
                tangon = this;
                entity = verify.user;
                michal = entity.id;
                zuuluu = _closure1_slot7;
                entity = zuuluu.getId;
                entity = entity.bind(zuuluu)();
                if(!(michal === entity)) { _fun00022_ip = 173; continue _fun00021 }
 40:
                zuuluu = _closure1_slot9;
                michal = zuuluu.getGuild;
                entity = verify.guildId;
                oscard = michal.bind(zuuluu)(entity);
                entity = null;
                if(!(entity != oscard)) { _fun00022_ip = 173; continue _fun00021 }
 65:
                zuuluu = tangon.put;
                report = _closure1_slot2;
                michal = _closure1_slot3;
                entity = 8;
                michal = michal[entity];
                entity = undefined;
                report = report.bind(entity)(michal);
                michal = report.attachSerializedData;
                entity = {};
                option = {};
                offset = verify.roles;
                option['roles'] = offset;
                verify = verify.user;
                verify = verify.id;
                option['userId'] = verify;
                entity['member'] = option;
                verify = _closure1_slot9;
                option = verify.getRoles;
                golfie = oscard.id;
                golfie = option.bind(verify)(golfie);
                entity['roles'] = golfie;
                michal = michal.bind(report)(oscard, entity);
                entity = argBar;
                entity = zuuluu.bind(tangon)(michal, entity);
 173:
                entity = undefined;
                return entity;
            }
        };
        report['value'] = golfie;
        entity[10] = report;
        report = {};
        golfie = 'resetInMemoryState';
        report['key'] = golfie;
        golfie = function() { // Original name: value
            entity = undefined;
            return entity;
        };
        report['value'] = golfie;
        entity[11] = report;
        report = {};
        golfie = 'putOne';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            _fun00023: for(var _fun00024_ip = 0; ; ) switch(_fun00024_ip) {
 0:
                option = argFoo;
                report = this;
                zuuluu = option.members;
                michal = zuuluu.find;
                entity = function(argFoo) {
                    entity = argFoo;
                    entity = entity.user;
                    michal = entity.id;
                    zuuluu = _closure1_slot7;
                    entity = zuuluu.getId;
                    entity = entity.bind(zuuluu)();
                    entity = michal === entity;
                    return entity;
                };
                verify = michal.bind(zuuluu)(entity);
                zuuluu = _closure1_slot9;
                michal = zuuluu.getGuild;
                entity = option.id;
                golfie = michal.bind(zuuluu)(entity);
                zuuluu = _closure1_slot2;
                yankee = _closure1_slot3;
                michal = 8;
                tangon = yankee[michal];
                entity = undefined;
                oscard = zuuluu.bind(entity)(tangon);
                tangon = oscard.attachSerializedData;
                michal = yankee[michal];
                zuuluu = zuuluu.bind(entity)(michal);
                michal = zuuluu.fromServer;
                zuuluu = michal.bind(zuuluu)(option, golfie);
                michal = {};
                yankee = option.roles;
                golfie = global;
                golfie = golfie.Array;
                golfie = yankee instanceof golfie;
                if(golfie) { _fun00024_ip = 131; continue _fun00023 }
 123:
                golfie = option.roles;
                _fun00024_ip = 174; continue _fun00023;
 131:
                yankee = _closure1_slot0;
                romeon = _closure1_slot3;
                offset = 9;
                offset = romeon[offset];
                romeon = yankee.bind(entity)(offset);
                yankee = romeon.sortServerRoles;
                offset = option.id;
                option = option.roles;
                golfie = yankee.bind(romeon)(offset, option);
 174:
                michal['roles'] = golfie;
                golfie = null;
                option = golfie != verify;
                golfie = undefined;
                if(!option) { _fun00024_ip = 220; continue _fun00023 }
 190:
                option = {};
                offset = verify.user;
                offset = offset.id;
                option['userId'] = offset;
                verify = verify.roles;
                option['roles'] = verify;
                golfie = option;
 220:
                michal['member'] = golfie;
                tangon = tangon.bind(oscard)(zuuluu, michal);
                zuuluu = report.put;
                michal = argBar;
                michal = zuuluu.bind(report)(tangon, michal);
                return entity;
            }
        };
        report['value'] = golfie;
        entity[12] = report;
        report = {};
        golfie = 'put';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            zuuluu = _closure1_slot1;
            michal = _closure1_slot3;
            entity = 7;
            michal = michal[entity];
            entity = undefined;
            tangon = zuuluu.bind(entity)(michal);
            zuuluu = tangon.guildsTransaction;
            michal = argBar;
            tangon = zuuluu.bind(tangon)(michal);
            zuuluu = tangon.put;
            michal = argFoo;
            michal = zuuluu.bind(tangon)(michal);
            return entity;
        };
        report['value'] = golfie;
        entity[13] = report;
        report = {};
        golfie = 'delete';
        report['key'] = golfie;
        golfie = function(argFoo, argBar) { // Original name: value
            zuuluu = _closure1_slot1;
            michal = _closure1_slot3;
            entity = 7;
            michal = michal[entity];
            entity = undefined;
            tangon = zuuluu.bind(entity)(michal);
            zuuluu = tangon.guildsTransaction;
            michal = argBar;
            tangon = zuuluu.bind(tangon)(michal);
            zuuluu = tangon.delete;
            michal = argFoo;
            michal = zuuluu.bind(tangon)(michal);
            return entity;
        };
        report['value'] = golfie;
        entity[14] = report;
        report = {};
        golfie = 'clear';
        report['key'] = golfie;
        oscard = function(argFoo) { // Original name: value
            zuuluu = _closure1_slot1;
            michal = _closure1_slot3;
            entity = 7;
            michal = michal[entity];
            entity = undefined;
            tangon = zuuluu.bind(entity)(michal);
            zuuluu = tangon.guildsTransaction;
            michal = argFoo;
            zuuluu = zuuluu.bind(tangon)(michal);
            michal = zuuluu.delete;
            michal = michal.bind(zuuluu)();
            return entity;
        };
        report['value'] = oscard;
        entity[15] = report;
        entity = tangon.bind(michal)(zuuluu, entity);
        return entity;
    };
    michal = michal.bind(entity)();
    tangon = michal.prototype;
    tangon = Object.create(tangon, {constructor: {value: michal}});
    foxtra = tangon;
    michal = new foxtra[michal](romeon);
    michal = michal instanceof Object ? michal : tangon;
    tangon = 10;
    tangon = oscard[tangon];
    oscard = report.bind(entity)(tangon);
    report = oscard.fileFinishedImporting;
    tangon = 'modules/app_database/modules/Guilds.tsx';
    tangon = report.bind(oscard)(tangon);
    zuuluu['default'] = michal;
    return entity;
})();