// app/modules/guild_role_subscriptions/native/components/FormEmojiPicker.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    backup = argBaz;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = backup;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    golf = oscar[entity];
    tango = argCorge;
    entity = undefined;
    tango = tango.bind(entity)(golf);
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    yankee = tango.Fonts;
    tango = 2;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.EmojiIntention;
    var _closure1_slot3 = tango;
    tango = 3;
    golf = oscar[tango];
    golf = report.bind(entity)(golf);
    golf = golf.jsx;
    var _closure1_slot4 = golf;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsxs;
    var _closure1_slot5 = tango;
    tango = 4;
    tango = oscar[tango];
    options = report.bind(entity)(tango);
    golf = options.createStyles;
    tango = {};
    verify = {'alignItems': 'center', 'flexDirection': 'row'};
    tango['container'] = verify;
    verify = {'marginStart': 8, 'flexGrow': 1};
    tango['content'] = verify;
    verify = {};
    romeo = 5;
    offset = oscar[romeo];
    output = backup.bind(entity)(offset);
    sizing = yankee.PRIMARY_MEDIUM;
    offset = 6;
    foxtrot = oscar[offset];
    foxtrot = backup.bind(entity)(foxtrot);
    foxtrot = foxtrot.colors;
    kilo = foxtrot.TEXT_MUTED;
    foxtrot = 16;
    echo = output.bind(entity)(sizing, kilo, foxtrot);
    update = verify;
    kilo = copyDataProperties(update, echo);
    tango['placeholder'] = verify;
    verify = {};
    romeo = oscar[romeo];
    romeo = backup.bind(entity)(romeo);
    yankee = yankee.PRIMARY_MEDIUM;
    offset = oscar[offset];
    offset = backup.bind(entity)(offset);
    offset = offset.colors;
    offset = offset.TEXT_NORMAL;
    echo = romeo.bind(entity)(yankee, offset, foxtrot);
    update = verify;
    offset = copyDataProperties(update, echo);
    tango['text'] = verify;
    tango = golf.bind(options)(tango);
    var _closure1_slot6 = tango;
    tango = 19;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/guild_role_subscriptions/native/components/FormEmojiPicker.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: FormEmojiPicker
        _fun127231: for(var _fun127231_ip = 0; ; ) switch(_fun127231_ip) {
 0:
            zulu = argFoo;
            mike = zulu.emoji;
            entity = mike.emojiId;
            mike = mike.emojiName;
            offset = zulu.guildId;
            var _closure2_slot0 = offset;
            zulu = zulu.onChange;
            var _closure2_slot1 = zulu;
            zulu = _closure1_slot6;
            tango = undefined;
            backup = zulu.bind(tango)();
            golf = _closure1_slot1;
            options = _closure1_slot2;
            zulu = 7;
            zulu = options[zulu];
            zulu = golf.bind(tango)(zulu);
            verify = zulu.bind(tango)();
            yankee = null;
            if(!(yankee == entity)) { _fun127231_ip = 133; continue _fun127231 }
 85:
            golf = _closure1_slot1;
            options = _closure1_slot2;
            zulu = 8;
            zulu = options[zulu];
            options = golf.bind(tango)(zulu);
            golf = options.convertSurrogateToName;
            romeo = yankee != mike;
            zulu = '';
            if(!romeo) { _fun127231_ip = 125; continue _fun127231 }
 122:
            zulu = mike;
 125:
            mike = false;
            entity = golf.bind(options)(zulu, mike);
 133:
            zulu = _closure1_slot0;
            golf = _closure1_slot2;
            mike = 9;
            mike = golf[mike];
            zulu = zulu.bind(tango)(mike);
            mike = zulu.useEmojiById;
            romeo = mike.bind(zulu)(offset, entity);
            options = entity;
            if(!(yankee == options)) { _fun127231_ip = 231; continue _fun127231 }
 172:
            zulu = _closure1_slot4;
            foxtrot = _closure1_slot1;
            kilo = _closure1_slot2;
            entity = 11;
            entity = kilo[entity];
            mike = foxtrot.bind(tango)(entity);
            entity = {};
            golf = 'contain';
            entity['resizeMode'] = golf;
            golf = 12;
            golf = kilo[golf];
            golf = foxtrot.bind(tango)(golf);
            entity['source'] = golf;
            golf = zulu.bind(tango)(mike, entity);
            _fun127231_ip = 271; continue _fun127231;
 231:
            zulu = _closure1_slot4;
            mike = _closure1_slot1;
            foxtrot = _closure1_slot2;
            entity = 10;
            entity = foxtrot[entity];
            mike = mike.bind(tango)(entity);
            entity = {};
            entity['guildId'] = offset;
            entity['id'] = options;
            golf = zulu.bind(tango)(mike, entity);
 271:
            zulu = _closure1_slot5;
            mike = _closure1_slot1;
            offset = _closure1_slot2;
            entity = 13;
            entity = offset[entity];
            mike = mike.bind(tango)(entity);
            entity = {};
            foxtrot = backup.container;
            options = new Array(2);
            options[0] = foxtrot;
            verify = verify.textInput;
            options[1] = verify;
            entity['style'] = options;
            options = 'link';
            entity['accessibilityRole'] = options;
            report = function() { // Original name: onPress
                zulu = _closure1_slot0;
                mike = _closure1_slot2;
                entity = 14;
                mike = mike[entity];
                entity = undefined;
                tango = zulu.bind(entity)(mike);
                zulu = tango.openEmojiPickerActionSheet;
                mike = {};
                oscar = _closure2_slot0;
                mike['guildId'] = oscar;
                oscar = function(argFoo) { // Original name: onPressEmoji
                    _fun127233: for(var _fun127233_ip = 0; ; ) switch(_fun127233_ip) {
 0:
                        entity = argFoo;
                        mike = entity.id;
                        tango = null;
                        if(!(tango == mike)) { _fun127233_ip = 61; continue _fun127233 }
 14:
                        mike = entity.optionallyDiverseSequence;
                        if(!(tango != mike)) { _fun127233_ip = 95; continue _fun127233 }
 24:
                        zulu = _closure2_slot1;
                        if(!(tango != zulu)) { _fun127233_ip = 95; continue _fun127233 }
 35:
                        report = _closure2_slot1;
                        zulu = {};
                        mike = entity.optionallyDiverseSequence;
                        zulu['emojiName'] = mike;
                        mike = undefined;
                        mike = report.bind(mike)(zulu);
                        _fun127233_ip = 95; continue _fun127233;
 61:
                        zulu = _closure2_slot1;
                        if(!(tango != zulu)) { _fun127233_ip = 95; continue _fun127233 }
 72:
                        zulu = _closure2_slot1;
                        mike = {};
                        entity = entity.id;
                        mike['emojiId'] = entity;
                        entity = undefined;
                        entity = zulu.bind(entity)(mike);
 95:
                        entity = undefined;
                        return entity;
                    }
                };
                mike['onPressEmoji'] = oscar;
                report = _closure1_slot3;
                report = report.GUILD_ROLE_BENEFIT_EMOJI;
                mike['pickerIntention'] = report;
                mike = zulu.bind(tango)(mike);
                return entity;
            };
            entity['onPress'] = report;
            report = new Array(3);
            report[0] = golf;
            verify = _closure1_slot4;
            options = _closure1_slot0;
            golf = 15;
            golf = offset[golf];
            golf = options.bind(tango)(golf);
            options = golf.LegacyText;
            golf = {};
            foxtrot = backup.content;
            offset = new Array(2);
            offset[0] = foxtrot;
            if(!(yankee == romeo)) { _fun127231_ip = 404; continue _fun127231 }
 396:
            foxtrot = backup.placeholder;
            _fun127231_ip = 409; continue _fun127231;
 404:
            foxtrot = backup.text;
 409:
            offset[1] = foxtrot;
            golf['style'] = offset;
            foxtrot = yankee == romeo;
            offset = undefined;
            if(foxtrot) { _fun127231_ip = 432; continue _fun127231 }
 426:
            offset = romeo.allNamesString;
 432:
            if(!(yankee == offset)) { _fun127231_ip = 491; continue _fun127231 }
 436:
            backup = _closure1_slot0;
            kilo = _closure1_slot2;
            yankee = 16;
            romeo = kilo[yankee];
            romeo = backup.bind(tango)(romeo);
            foxtrot = romeo.intl;
            romeo = foxtrot.string;
            yankee = kilo[yankee];
            yankee = backup.bind(tango)(yankee);
            yankee = yankee.t;
            yankee = yankee.gXAN3N;
            offset = romeo.bind(foxtrot)(yankee);
 491:
            golf['children'] = offset;
            golf = verify.bind(tango)(options, golf);
            report[1] = golf;
            options = _closure1_slot4;
            offset = _closure1_slot1;
            yankee = _closure1_slot2;
            verify = 17;
            oscar = yankee[verify];
            golf = offset.bind(tango)(oscar);
            oscar = {};
            verify = yankee[verify];
            verify = offset.bind(tango)(verify);
            verify = verify.Sizes;
            verify = verify.MEDIUM;
            oscar['size'] = verify;
            verify = 18;
            verify = yankee[verify];
            verify = offset.bind(tango)(verify);
            oscar['source'] = verify;
            oscar = options.bind(tango)(golf, oscar);
            report[2] = oscar;
            entity['children'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();