// app/modules/main_tabs_v2/useRecipientsLabel.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    golf = argBaz;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = golf;
    var _closure1_slot2 = oscar;
    entity = global;
    verify = entity.Object;
    options = verify.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = options.bind(verify)(zulu, entity, tango);
    entity = 0;
    options = oscar[entity];
    tango = argCorge;
    entity = undefined;
    tango = tango.bind(entity)(options);
    var _closure1_slot3 = tango;
    tango = 1;
    tango = oscar[tango];
    tango = golf.bind(entity)(tango);
    var _closure1_slot4 = tango;
    tango = 6;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/main_tabs_v2/useRecipientsLabel.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: useRecipientsLabel
        oscar = argFoo;
        var _closure2_slot0 = oscar;
        report = _closure1_slot0;
        tango = _closure1_slot2;
        zulu = 3;
        tango = tango[zulu];
        zulu = undefined;
        golf = report.bind(zulu)(tango);
        report = golf.useStateFromStoresArray;
        zulu = _closure1_slot4;
        tango = new Array(1);
        tango[0] = zulu;
        zulu = function() {
            mike = _closure2_slot0;
            tango = mike.recipients;
            zulu = tango.map;
            mike = function(argFoo) {
                zulu = _closure1_slot4;
                mike = zulu.getUser;
                entity = argFoo;
                entity = mike.bind(zulu)(entity);
                return entity;
            };
            tango = zulu.bind(tango)(mike);
            zulu = tango.filter;
            oscar = _closure1_slot0;
            report = _closure1_slot2;
            mike = 4;
            report = report[mike];
            mike = undefined;
            mike = oscar.bind(mike)(report);
            mike = mike.isNotNullish;
            zulu = zulu.bind(tango)(mike);
            mike = zulu.map;
            entity = function(argFoo) {
                zulu = _closure1_slot1;
                mike = _closure1_slot2;
                entity = 5;
                mike = mike[entity];
                entity = undefined;
                zulu = zulu.bind(entity)(mike);
                mike = zulu.getName;
                entity = argFoo;
                entity = mike.bind(zulu)(entity);
                return entity;
            };
            entity = mike.bind(zulu)(entity);
            return entity;
        };
        report = report.bind(golf)(tango, zulu);
        var _closure2_slot1 = report;
        tango = _closure1_slot3;
        zulu = tango.useMemo;
        mike = new Array(2);
        mike[0] = oscar;
        mike[1] = report;
        entity = function() {
            _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
                entity = _closure2_slot0;
                report = entity.name;
                tango = '';
                entity = null;
                if(!(tango !== report)) { _fun00002_ip = 447; continue _fun00001 }
 25:
                options = _closure2_slot1;
                zulu = options.length;
                golf = 0;
                mike = null;
                if(!(golf !== zulu)) { _fun00002_ip = 444; continue _fun00001 }
 45:
                zulu = options.length;
                verify = 1;
                if(!(verify !== zulu)) { _fun00002_ip = 370; continue _fun00001 }
 60:
                zulu = options.length;
                yankee = 2;
                if(!(yankee !== zulu)) { _fun00002_ip = 288; continue _fun00001 }
 75:
                zulu = options.length;
                tango = 3;
                if(!(tango !== zulu)) { _fun00002_ip = 194; continue _fun00001 }
 87:
                zulu = options.length;
                offset = zulu - tango;
                romeo = _closure1_slot0;
                zulu = _closure1_slot2;
                report = zulu[yankee];
                tango = undefined;
                report = romeo.bind(tango)(report);
                oscar = report.intl;
                report = oscar.formatToPlainString;
                zulu = zulu[yankee];
                zulu = romeo.bind(tango)(zulu);
                zulu = zulu.t;
                tango = zulu.VYfueX;
                zulu = {};
                romeo = options[golf];
                zulu['first'] = romeo;
                romeo = options[verify];
                zulu['second'] = romeo;
                romeo = options[yankee];
                zulu['third'] = romeo;
                zulu['count'] = offset;
                mike = report.bind(oscar)(tango, zulu);
                _fun00002_ip = 444; continue _fun00001;
 194:
                offset = _closure1_slot0;
                zulu = _closure1_slot2;
                report = zulu[yankee];
                tango = undefined;
                report = offset.bind(tango)(report);
                oscar = report.intl;
                report = oscar.formatToPlainString;
                zulu = zulu[yankee];
                zulu = offset.bind(tango)(zulu);
                zulu = zulu.t;
                tango = zulu.QDB5en;
                zulu = {};
                offset = options[golf];
                zulu['first'] = offset;
                offset = options[verify];
                zulu['second'] = offset;
                offset = options[yankee];
                zulu['third'] = offset;
                mike = report.bind(oscar)(tango, zulu);
                _fun00002_ip = 444; continue _fun00001;
 288:
                offset = _closure1_slot0;
                zulu = _closure1_slot2;
                report = zulu[yankee];
                tango = undefined;
                report = offset.bind(tango)(report);
                oscar = report.intl;
                report = oscar.formatToPlainString;
                zulu = zulu[yankee];
                zulu = offset.bind(tango)(zulu);
                zulu = zulu.t;
                tango = zulu.gwRP0d;
                zulu = {};
                offset = options[golf];
                zulu['first'] = offset;
                verify = options[verify];
                zulu['second'] = verify;
                mike = report.bind(oscar)(tango, zulu);
                _fun00002_ip = 444; continue _fun00001;
 370:
                verify = _closure1_slot0;
                offset = _closure1_slot2;
                zulu = 2;
                report = offset[zulu];
                tango = undefined;
                report = verify.bind(tango)(report);
                oscar = report.intl;
                report = oscar.formatToPlainString;
                zulu = offset[zulu];
                zulu = verify.bind(tango)(zulu);
                zulu = zulu.t;
                tango = zulu.J+Wpsr;
                zulu = {};
                golf = options[golf];
                zulu['first'] = golf;
                mike = report.bind(oscar)(tango, zulu);
 444:
                entity = mike;
 447:
                return entity;
            }
        };
        entity = zulu.bind(tango)(entity, mike);
        return entity;
    };
    zulu['useRecipientsLabel'] = mike;
    return entity;
})();