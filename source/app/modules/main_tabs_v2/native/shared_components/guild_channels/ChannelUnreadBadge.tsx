// app/modules/main_tabs_v2/native/shared_components/guild_channels/ChannelUnreadBadge.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    entity = argBaz;
    var _closure1_slot1 = entity;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    golf = oscar[entity];
    tango = argCorge;
    entity = undefined;
    golf = tango.bind(entity)(golf);
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.View;
    var _closure1_slot3 = tango;
    tango = 2;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.MUTED_OPACITY_CONTENT;
    var _closure1_slot4 = tango;
    tango = 3;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.UnreadSetting;
    var _closure1_slot5 = tango;
    tango = 4;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsx;
    var _closure1_slot6 = tango;
    tango = 5;
    tango = oscar[tango];
    verify = report.bind(entity)(tango);
    options = verify.createStyles;
    tango = {};
    offset = {'flexGrow': 0, 'flexShrink': 0, 'position': 'absolute'};
    tango['unreadBadge'] = offset;
    offset = {};
    yankee = -16;
    offset['marginLeft'] = yankee;
    tango['unreadBadgePanel'] = offset;
    tango = options.bind(verify)(tango);
    var _closure1_slot7 = tango;
    tango = golf.memo;
    mike = function(argFoo) { // Original name: ChannelUnreadBadge
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            entity = argFoo;
            mike = entity.unread;
            backup = entity.resolvedUnreadSetting;
            romeo = entity.muted;
            verify = entity.isThread;
            offset = entity.layout;
            golf = entity.launchpad;
            kilo = entity.panelVariant;
            report = undefined;
            if(!(kilo === report)) { _fun00002_ip = 53; continue _fun00001 }
 51:
            kilo = false;
 53:
            entity = _closure1_slot7;
            foxtrot = entity.bind(report)();
            zulu = _closure1_slot0;
            tango = _closure1_slot2;
            options = 6;
            entity = tango[options];
            oscar = zulu.bind(report)(entity);
            entity = oscar.getLayoutStyles;
            golf = entity.bind(oscar)(offset, golf);
            entity = 7;
            entity = tango[entity];
            zulu = zulu.bind(report)(entity);
            entity = zulu.useFontScale;
            sizing = entity.bind(zulu)();
            entity = null;
            if(!mike) { _fun00002_ip = 366; continue _fun00001 }
 126:
            tango = _closure1_slot6;
            zulu = _closure1_slot3;
            mike = {};
            offset = foxtrot.unreadBadge;
            oscar = new Array(4);
            oscar[0] = offset;
            offset = undefined;
            if(!kilo) { _fun00002_ip = 161; continue _fun00001 }
 155:
            offset = foxtrot.unreadBadgePanel;
 161:
            oscar[1] = offset;
            offset = golf.unreadBadge;
            if(verify) { _fun00002_ip = 181; continue _fun00001 }
 174:
            verify = offset.position;
            _fun00002_ip = 187; continue _fun00001;
 181:
            verify = offset.positionThread;
 187:
            oscar[2] = verify;
            offset = _closure1_slot0;
            foxtrot = _closure1_slot2;
            options = foxtrot[options];
            verify = offset.bind(report)(options);
            options = verify.makeSizeStyle;
            golf = golf.unreadBadge;
            golf = golf.size;
            golf = options.bind(verify)(golf);
            oscar[3] = golf;
            mike['style'] = oscar;
            options = _closure1_slot6;
            golf = _closure1_slot1;
            verify = 8;
            oscar = foxtrot[verify];
            golf = golf.bind(report)(oscar);
            oscar = {};
            oscar['classic'] = kilo;
            verify = foxtrot[verify];
            verify = offset.bind(report)(verify);
            foxtrot = verify.CHANNEL_BADGE_SIZE;
            verify = global;
            kilo = verify.Math;
            offset = kilo.max;
            verify = 1;
            offset = offset.bind(kilo)(sizing, verify);
            offset = foxtrot * offset;
            oscar['size'] = offset;
            offset = {};
            foxtrot = _closure1_slot5;
            foxtrot = foxtrot.ALL_MESSAGES;
            if(!(backup === foxtrot)) { _fun00002_ip = 329; continue _fun00001 }
 326:
            if(!romeo) { _fun00002_ip = 333; continue _fun00001 }
 329:
            verify = _closure1_slot4;
 333:
            offset['opacity'] = verify;
            verify = new Array(1);
            verify[0] = offset;
            oscar['badgeStyle'] = verify;
            oscar = options.bind(report)(golf, oscar);
            mike['children'] = oscar;
            entity = tango.bind(report)(zulu, mike);
 366:
            return entity;
        }
    };
    mike = tango.bind(golf)(mike);
    tango = 9;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'modules/main_tabs_v2/native/shared_components/guild_channels/ChannelUnreadBadge.tsx';
    tango = report.bind(oscar)(tango);
    zulu['default'] = mike;
    return entity;
})();