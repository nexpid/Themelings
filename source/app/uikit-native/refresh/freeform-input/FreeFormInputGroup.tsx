// app/uikit-native/refresh/freeform-input/FreeFormInputGroup.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    entity = argBaz;
    var _closure1_slot1 = entity;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    golf = oscar[entity];
    tango = argCorge;
    entity = undefined;
    golf = tango.bind(entity)(golf);
    var _closure1_slot3 = golf;
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.View;
    var _closure1_slot4 = tango;
    tango = 2;
    options = oscar[tango];
    options = report.bind(entity)(options);
    options = options.jsx;
    var _closure1_slot5 = options;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsxs;
    var _closure1_slot6 = tango;
    tango = 3;
    tango = oscar[tango];
    verify = report.bind(entity)(tango);
    options = verify.createStyles;
    tango = {};
    offset = {};
    yankee = 8;
    offset['marginBottom'] = yankee;
    tango['label'] = offset;
    offset = {'flexGrow': 1, 'marginBottom': 8};
    tango['input'] = offset;
    offset = {};
    offset['marginBottom'] = yankee;
    tango['error'] = offset;
    offset = {};
    offset['marginBottom'] = yankee;
    tango['hint'] = offset;
    tango = options.bind(verify)(tango);
    var _closure1_slot7 = tango;
    tango = golf.forwardRef;
    mike = function(argFoo, argBar) {
        _fun00001: for(var _fun00002_ip = 0; ; ) switch(_fun00002_ip) {
 0:
            oscar = argFoo;
            output = oscar.style;
            kilo = oscar.label;
            backup = oscar.error;
            sizing = oscar.value;
            foxtrot = oscar.hint;
            update = oscar.textStyle;
            zulu = oscar.enableAndroidSanitizedInputWorkaround;
            tango = undefined;
            if(!(zulu === tango)) { _fun00002_ip = 49; continue _fun00001 }
 47:
            zulu = false;
 49:
            golf = oscar.secureTextEntry;
            mike = oscar.keyboardType;
            vacuum = oscar.accessibilityLabel;
            report = {'style': 0, 'label': 0, 'error': 0, 'value': 0, 'hint': 0, 'textStyle': 0, 'enableAndroidSanitizedInputWorkaround': 0, 'secureTextEntry': 0, 'keyboardType': 0, 'accessibilityLabel': 0};
            offset = null;
            papa = report;
            context = null;
            entity = silentSetPrototypeOf(papa, context);
            papa = {};
            context = oscar;
            record = report;
            report = copyDataProperties(papa, context, record);
            oscar = _closure1_slot7;
            echo = oscar.bind(tango)();
            oscar = zulu;
            if(!oscar) { _fun00002_ip = 145; continue _fun00001 }
 116:
            verify = _closure1_slot0;
            yankee = _closure1_slot2;
            options = 4;
            options = yankee[options];
            verify = verify.bind(tango)(options);
            options = verify.isAndroid;
            oscar = options.bind(verify)();
 145:
            if(oscar) { _fun00002_ip = 151; continue _fun00001 }
 148:
            oscar = golf;
 151:
            if(!zulu) { _fun00002_ip = 192; continue _fun00001 }
 154:
            golf = _closure1_slot0;
            options = _closure1_slot2;
            zulu = 4;
            zulu = options[zulu];
            golf = golf.bind(tango)(zulu);
            zulu = golf.isAndroid;
            zulu = zulu.bind(golf)();
            golf = 'visible-password';
            if(zulu) { _fun00002_ip = 195; continue _fun00001 }
 192:
            golf = mike;
 195:
            options = _closure1_slot3;
            zulu = options.useContext;
            verify = _closure1_slot0;
            yankee = _closure1_slot2;
            mike = 5;
            mike = yankee[mike];
            mike = verify.bind(tango)(mike);
            mike = mike.RedesignCompatContext;
            mike = zulu.bind(options)(mike);
            zulu = options.useId;
            control = zulu.bind(options)();
            if(mike) { _fun00002_ip = 672; continue _fun00001 }
 252:
            options = _closure1_slot6;
            zulu = _closure1_slot4;
            mike = {};
            mike['style'] = output;
            verify = offset != kilo;
            yankee = null;
            if(!verify) { _fun00002_ip = 325; continue _fun00001 }
 275:
            result = _closure1_slot5;
            romeo = _closure1_slot1;
            source = _closure1_slot2;
            verify = 8;
            verify = source[verify];
            romeo = romeo.bind(tango)(verify);
            verify = {};
            source = echo.label;
            verify['style'] = source;
            verify['nativeID'] = control;
            verify['children'] = kilo;
            yankee = result.bind(tango)(romeo, verify);
 325:
            verify = new Array(4);
            verify[0] = yankee;
            result = _closure1_slot5;
            romeo = _closure1_slot1;
            source = _closure1_slot2;
            yankee = 9;
            yankee = source[yankee];
            romeo = romeo.bind(tango)(yankee);
            yankee = {};
            papa = yankee;
            context = report;
            source = copyDataProperties(papa, context);
            if(!(offset == vacuum)) { _fun00002_ip = 417; continue _fun00001 }
 373:
            if(!(offset != kilo)) { _fun00002_ip = 411; continue _fun00001 }
 377:
            sequence = _closure1_slot0;
            config = _closure1_slot2;
            source = 4;
            source = config[source];
            sequence = sequence.bind(tango)(source);
            source = sequence.isAndroid;
            sequence = source.bind(sequence)();
            source = undefined;
            if(sequence) { _fun00002_ip = 414; continue _fun00001 }
 411:
            source = kilo;
 414:
            vacuum = source;
 417:
            source = 'accessibilityLabel';
            yankee[source] = vacuum;
            source = 'accessibilityLabelledBy';
            yankee[source] = control;
            control = offset != backup;
            source = 'error';
            yankee[source] = control;
            control = argBar;
            source = 'ref';
            yankee[source] = control;
            source = 'value';
            yankee[source] = sizing;
            source = 'secureTextEntry';
            yankee[source] = oscar;
            source = 'keyboardType';
            yankee[source] = golf;
            control = echo.input;
            source = new Array(2);
            source[0] = control;
            source[1] = update;
            update = 'style';
            yankee[update] = source;
            yankee = result.bind(tango)(romeo, yankee);
            verify[1] = yankee;
            romeo = offset != backup;
            yankee = null;
            if(!romeo) { _fun00002_ip = 578; continue _fun00001 }
 533:
            update = _closure1_slot5;
            result = _closure1_slot1;
            source = _closure1_slot2;
            romeo = 10;
            romeo = source[romeo];
            result = result.bind(tango)(romeo);
            romeo = {};
            source = echo.error;
            romeo['style'] = source;
            romeo['children'] = backup;
            yankee = update.bind(tango)(result, romeo);
 578:
            verify[2] = yankee;
            yankee = offset != foxtrot;
            offset = null;
            if(!yankee) { _fun00002_ip = 656; continue _fun00001 }
 591:
            result = _closure1_slot5;
            romeo = _closure1_slot0;
            update = _closure1_slot2;
            yankee = 11;
            yankee = update[yankee];
            yankee = romeo.bind(tango)(yankee);
            romeo = yankee.Text;
            yankee = {'style': null, 'variant': 'text-xs/medium', 'color': 'text-muted'};
            echo = echo.hint;
            yankee['style'] = echo;
            yankee['children'] = foxtrot;
            offset = result.bind(tango)(romeo, yankee);
 656:
            verify[3] = offset;
            mike['children'] = verify;
            mike = options.bind(tango)(zulu, mike);
            return mike;
 672:
            romeo = report.placeholder;
            options = report.onChangeText;
            verify = report.clearButtonVisibility;
            zulu = _closure1_slot5;
            offset = _closure1_slot0;
            yankee = _closure1_slot2;
            entity = 6;
            entity = yankee[entity];
            entity = offset.bind(tango)(entity);
            mike = entity.TextInput;
            entity = {};
            entity['containerStyle'] = output;
            entity['value'] = sizing;
            entity['label'] = kilo;
            entity['errorMessage'] = backup;
            entity['description'] = foxtrot;
            entity['placeholder'] = romeo;
            entity['onChange'] = options;
            options = 7;
            options = yankee[options];
            options = offset.bind(tango)(options);
            options = options.ClearButtonVisibility;
            options = options.WITH_CONTENT;
            options = verify !== options;
            entity['isClearable'] = options;
            entity['keyboardType'] = golf;
            entity['secureTextEntry'] = oscar;
            report = report.autoCapitalize;
            entity['autoCapitalize'] = report;
            entity = zulu.bind(tango)(mike, entity);
            return entity;
        }
    };
    mike = tango.bind(golf)(mike);
    tango = 12;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'uikit-native/refresh/freeform-input/FreeFormInputGroup.tsx';
    tango = report.bind(oscar)(tango);
    zulu['default'] = mike;
    return entity;
})();