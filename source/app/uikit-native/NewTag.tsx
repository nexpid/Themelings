// app/uikit-native/NewTag.tsx
export default (function(argFoo, argBar, argBaz, argCorge, _, argFred, argPlugh) {
    report = argBar;
    yankee = argBaz;
    zulu = argFred;
    oscar = argPlugh;
    var _closure1_slot0 = report;
    var _closure1_slot1 = yankee;
    var _closure1_slot2 = oscar;
    entity = global;
    options = entity.Object;
    golf = options.defineProperty;
    tango = {};
    entity = true;
    tango['value'] = entity;
    entity = '__esModule';
    entity = golf.bind(options)(zulu, entity, tango);
    entity = 0;
    golf = oscar[entity];
    tango = argCorge;
    entity = undefined;
    tango = tango.bind(entity)(golf);
    tango = 1;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    golf = tango.View;
    var _closure1_slot3 = golf;
    tango = tango.Platform;
    tango = 2;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.HorizontalGradient;
    var _closure1_slot4 = tango;
    tango = 3;
    tango = oscar[tango];
    tango = report.bind(entity)(tango);
    tango = tango.jsx;
    var _closure1_slot5 = tango;
    tango = 4;
    tango = oscar[tango];
    options = report.bind(entity)(tango);
    golf = options.createStyles;
    tango = {};
    verify = {'height': 'auto', 'backgroundColor': null, 'justifyContent': 'center', 'alignItems': 'center', 'paddingHorizontal': 4, 'marginBottom': 2};
    offset = 5;
    romeo = oscar[offset];
    romeo = yankee.bind(entity)(romeo);
    romeo = romeo.unsafe_rawColors;
    romeo = romeo.RED_400;
    verify['backgroundColor'] = romeo;
    offset = oscar[offset];
    offset = yankee.bind(entity)(offset);
    offset = offset.radii;
    offset = offset.round;
    verify['borderRadius'] = offset;
    tango['tagContainer'] = verify;
    verify = {};
    offset = 'uppercase';
    verify['textTransform'] = offset;
    tango['tagText'] = verify;
    tango = golf.bind(options)(tango);
    var _closure1_slot6 = tango;
    tango = 9;
    tango = oscar[tango];
    oscar = report.bind(entity)(tango);
    report = oscar.fileFinishedImporting;
    tango = 'uikit-native/NewTag.tsx';
    tango = report.bind(oscar)(tango);
    mike = function(argFoo) { // Original name: NewTag
        _fun76903: for(var _fun76903_ip = 0; ; ) switch(_fun76903_ip) {
 0:
            tango = argFoo;
            foxtrot = tango.containerStyle;
            offset = tango.textStyle;
            sizing = tango.variant;
            report = undefined;
            if(!(sizing === report)) { _fun76903_ip = 32; continue _fun76903 }
 26:
            sizing = 'heading-sm/semibold';
 32:
            kilo = tango.color;
            if(!(kilo === report)) { _fun76903_ip = 47; continue _fun76903 }
 41:
            kilo = 'always-white';
 47:
            entity = tango.gradient;
            if(!(entity === report)) { _fun76903_ip = 59; continue _fun76903 }
 57:
            entity = false;
 59:
            verify = tango.borderRadius;
            if(!(verify === report)) { _fun76903_ip = 102; continue _fun76903 }
 68:
            zulu = _closure1_slot1;
            oscar = _closure1_slot2;
            mike = 5;
            mike = oscar[mike];
            mike = zulu.bind(report)(mike);
            mike = mike.radii;
            verify = mike.sm;
 102:
            oscar = tango.colors;
            if(!(oscar === report)) { _fun76903_ip = 180; continue _fun76903 }
 111:
            golf = _closure1_slot1;
            options = _closure1_slot2;
            zulu = 5;
            mike = options[zulu];
            mike = golf.bind(report)(mike);
            mike = mike.unsafe_rawColors;
            yankee = mike.PREMIUM_TIER_2_PURPLE;
            mike = new Array(2);
            mike[0] = yankee;
            zulu = options[zulu];
            zulu = golf.bind(report)(zulu);
            zulu = zulu.unsafe_rawColors;
            zulu = zulu.PREMIUM_TIER_2_PINK;
            mike[1] = zulu;
            oscar = mike;
 180:
            zulu = {'containerStyle': 0, 'textStyle': 0, 'variant': 0, 'color': 0, 'gradient': 0, 'borderRadius': 0, 'colors': 0};
            control = null;
            vacuum = zulu;
            mike = silentSetPrototypeOf(vacuum, control);
            vacuum = {};
            control = tango;
            source = zulu;
            romeo = copyDataProperties(vacuum, control, source);
            mike = _closure1_slot6;
            yankee = mike.bind(report)();
            tango = _closure1_slot5;
            if(entity) { _fun76903_ip = 413; continue _fun76903 }
 236:
            mike = _closure1_slot3;
            entity = {};
            golf = yankee.tagContainer;
            zulu = new Array(2);
            zulu[0] = golf;
            zulu[1] = foxtrot;
            entity['style'] = zulu;
            echo = _closure1_slot0;
            update = _closure1_slot2;
            zulu = 7;
            zulu = update[zulu];
            zulu = echo.bind(report)(zulu);
            golf = zulu.Text;
            zulu = {};
            zulu['variant'] = sizing;
            zulu['color'] = kilo;
            vacuum = zulu;
            control = romeo;
            backup = copyDataProperties(vacuum, control);
            backup = yankee.tagText;
            output = new Array(2);
            output[0] = backup;
            output[1] = offset;
            backup = 'style';
            zulu[backup] = output;
            backup = 8;
            output = update[backup];
            output = echo.bind(report)(output);
            result = output.intl;
            output = result.string;
            backup = update[backup];
            backup = echo.bind(report)(backup);
            backup = backup.t;
            backup = backup.y2b7CA;
            output = output.bind(result)(backup);
            backup = 'children';
            zulu[backup] = output;
            zulu = tango.bind(report)(golf, zulu);
            entity['children'] = zulu;
            entity = tango.bind(report)(mike, entity);
            _fun76903_ip = 679; continue _fun76903;
 413:
            output = _closure1_slot1;
            backup = _closure1_slot2;
            mike = 6;
            mike = backup[mike];
            zulu = output.bind(report)(mike);
            mike = {};
            golf = {};
            golf['borderRadius'] = verify;
            verify = 5;
            verify = backup[verify];
            verify = output.bind(report)(verify);
            verify = verify.spacing;
            verify = verify.PX_4;
            golf['marginLeft'] = verify;
            mike['style'] = golf;
            golf = _closure1_slot4;
            verify = golf.START;
            mike['start'] = verify;
            golf = golf.END;
            mike['end'] = golf;
            mike['colors'] = oscar;
            golf = _closure1_slot3;
            oscar = {};
            output = yankee.tagContainer;
            verify = new Array(2);
            verify[0] = output;
            verify[1] = foxtrot;
            oscar['style'] = verify;
            foxtrot = _closure1_slot0;
            options = 7;
            options = backup[options];
            options = foxtrot.bind(report)(options);
            verify = options.Text;
            options = {};
            options['variant'] = sizing;
            options['color'] = kilo;
            vacuum = options;
            control = romeo;
            romeo = copyDataProperties(vacuum, control);
            romeo = yankee.tagText;
            yankee = new Array(2);
            yankee[0] = romeo;
            yankee[1] = offset;
            offset = 'style';
            options[offset] = yankee;
            offset = 8;
            yankee = backup[offset];
            yankee = foxtrot.bind(report)(yankee);
            romeo = yankee.intl;
            yankee = romeo.string;
            offset = backup[offset];
            offset = foxtrot.bind(report)(offset);
            offset = offset.t;
            offset = offset.y2b7CA;
            yankee = yankee.bind(romeo)(offset);
            offset = 'children';
            options[offset] = yankee;
            options = tango.bind(report)(verify, options);
            oscar['children'] = options;
            oscar = tango.bind(report)(golf, oscar);
            mike['children'] = oscar;
            entity = tango.bind(report)(zulu, mike);
 679:
            return entity;
        }
    };
    zulu['default'] = mike;
    return entity;
})();